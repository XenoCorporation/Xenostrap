...
APPLICATION_TARGET="$OUTPUT/Xenostrap.app"
DMG_TARGET="$OUTPUT/Xenostrap-$RID.dmg"
ARCHIVE_TARGET="$OUTPUT/Xenostrap-$RID.zip"
...
LOCK="$OUTPUT/.xenostrap-macos-$RID.lock"
...
trap cleanup EXIT
STAGE="$(mktemp -d "$OUTPUT/.xenostrap-macos.XXXXXX")"
PUBLISH="$STAGE/publish"
APPLICATION="$STAGE/Xenostrap.app"
DMG="$STAGE/Xenostrap-$RID.dmg"
ARCHIVE="$STAGE/Xenostrap-$RID.zip"
...
mkdir -p "$PUBLISH"
dotnet publish "$ROOT/src/Xenostrap.Cross/Xenostrap.Cross.csproj" -c Release -r "$RID" --self-contained true -o "$PUBLISH" -p:Version="$VERSION" -p:DebugType=none -p:DebugSymbols=false
...
hdiutil create -volname Xenostrap -srcfolder "$APPLICATION" -ov -format UDZO "$DMG"
