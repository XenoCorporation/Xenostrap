...
$WinProj   = Join-Path $Root 'src\Xenostrap.App\Xenostrap.csproj'
$CrossProj = Join-Path $Root 'src\Xenostrap.Cross\Xenostrap.Cross.csproj'
$Sln       = Join-Path $Root 'Xenostrap.sln'
...
$AllTargets = @(
    [pscustomobject]@{ Name='Windows';     Key='windows';     Kind='profile'; Rid=$null;        OutDir=(Join-Path $Out 'Windows');     Executable='Xenostrap.exe' }
    [pscustomobject]@{ Name='Linux-x64';   Key='linux-x64';   Kind='cross';   Rid='linux-x64';   OutDir=(Join-Path $Out 'Linux-x64');   Executable='Xenostrap' }
    [pscustomobject]@{ Name='Linux-arm64'; Key='linux-arm64'; Kind='cross';   Rid='linux-arm64'; OutDir=(Join-Path $Out 'Linux-arm64'); Executable='Xenostrap' }
    [pscustomobject]@{ Name='Linux-musl-x64';   Key='linux-musl-x64';   Kind='cross';   Rid='linux-musl-x64';   OutDir=(Join-Path $Out 'Linux-musl-x64');   Executable='Xenostrap' }
    [pscustomobject]@{ Name='Linux-musl-arm64'; Key='linux-musl-arm64'; Kind='cross';   Rid='linux-musl-arm64'; OutDir=(Join-Path $Out 'Linux-musl-arm64'); Executable='Xenostrap' }
    [pscustomobject]@{ Name='macOS-x64';   Key='osx-x64';     Kind='cross';   Rid='osx-x64';     OutDir=(Join-Path $Out 'macOS-x64');   Executable='Xenostrap' }
    [pscustomobject]@{ Name='macOS-arm64'; Key='osx-arm64';   Kind='cross';   Rid='osx-arm64';   OutDir=(Join-Path $Out 'macOS-arm64'); Executable='Xenostrap' }
)
...
Write-Host 'Xenostrap Build' -ForegroundColor Cyan
Write-Host "Output: $Out"
Write-Host ''

if (Get-Process -Name 'Xenostrap' -ErrorAction SilentlyContinue) {
    Write-Host 'Xenostrap.exe is running and locks the output file' -ForegroundColor Red
    Write-Host 'Close Xenostrap, then run this again'
    exit 1
}
