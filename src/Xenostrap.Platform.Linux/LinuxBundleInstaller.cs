...
public static class LinuxBundleInstaller
{
	...
	private const string ArchiveRoot = "Xenostrap";
	private const string ExecutableName = "Xenostrap";
	private const string BackupSuffix = ".xenostrap.previous";
	private const string MarkerSuffix = ".owner";
	private const string MarkerHeader = "Xenostrap Linux bundle backup\n";
	...
	public static string GetAssetName(string tag, string runtimeIdentifier)
	{
		...
		return $"Xenostrap_{version}_{runtimeIdentifier}.tar.gz";
	}
		...
		Action<string, string> atomicExchange)
	{
		...
		{
			throw new FileNotFoundException("The current Xenostrap executable is unavailable", executablePath);
		}
		...
		string stageDirectory = installDirectory + ".xenostrap.stage." + Guid.NewGuid().ToString("N");
	...
	{
		string firstProbe = Path.Combine(parentDirectory, ".xenostrap.exchange." + Guid.NewGuid().ToString("N"));
		string secondProbe = Path.Combine(parentDirectory, ".xenostrap.exchange." + Guid.NewGuid().ToString("N"));
		try
		{
	...
	private static void EnsureWritable(string directory)
	{
		...
		string probePath = Path.Combine(directory, ".xenostrap.write." + Guid.NewGuid().ToString("N"));
		try
		{
	...
	private static void ValidateCandidate(string stageDirectory, UnixFileMode? executableMode)
	{
		...
		{
			throw new InvalidDataException("The update bundle has no valid Xenostrap executable");
		}
		if (executableMode is null || (executableMode.Value & UnixFileMode.UserExecute) == 0)
		{
			throw new InvalidDataException("The update bundle has no executable Xenostrap program");
		}
	...
	private static void ValidateElfExecutable(string executablePath)
	{
		...
		{
			throw new InvalidDataException("The update bundle contains an invalid Xenostrap program");
		}
		...
		{
			throw new InvalidDataException("The update bundle contains a Xenostrap program for another architecture");
		}
