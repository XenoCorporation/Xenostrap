...
internal static class AutoSaveHelper
{
	...
	public static async Task SaveAsync(Stream stream, bool shouldCompress, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			path = Path.Combine(PathHelper.AutoSaves, "XenostrapClient Save " + DateTimeOffset.UtcNow.ToString("yyyyMMddTHHmmssfff") + " " + Guid.NewGuid().ToString("N") + ".rbxl.gz");
	...
	private static void TrimRetention()
	{
		try
		{
			...
			long storedBytes = 0;
			foreach (FileInfo file in new DirectoryInfo(PathHelper.AutoSaves).EnumerateFiles("XenostrapClient Save *.rbxl.gz"))
			{
