...
public class Watcher : IDisposable
{
	...
	private void InitializeIntegrations()
	{
		...
		bool flag = !string.IsNullOrEmpty(Environment.GetEnvironmentVariable("XENOSTRAP_STATUS_FILE"));
		...
		if ((enableActivityTracking || App.LaunchSettings.TestModeFlag.Active) && Xenostrap.Utility.Platform.SupportsTrayIcon)
	...
	private void EnableActivityIntegrations()
	{
		...
		if (_notifyIcon == null)
		{
			if (Xenostrap.Utility.Platform.SupportsTrayIcon)
			_notifyIcon = new NotifyIconWrapper(this);
		}
		else
		{
	...
	{
		RunRuntimeAction(Xenostrap.Integrations.Fullscreen.FakeExclusiveFullscreen.OnGameJoin, "FullscreenJoin");
		RunRuntimeAction(Xenostrap.Integrations.RiShade.RiShadeManager.OnGameJoin, "RiShadeJoin");
		RunRuntimeAction(Xenostrap.Integrations.AntiAliasing.AntiAliasingManager.OnGameJoin, "AntiAliasingJoin");
		RunOnApplicationDispatcher(EnsureRuntimeSessionWindows);
	}

	private void OnRuntimeGameLeave(object? sender, EventArgs e)
	{
		RunRuntimeAction(Xenostrap.Integrations.Fullscreen.FakeExclusiveFullscreen.OnGameLeave, "FullscreenLeave");
		RunRuntimeAction(Xenostrap.Integrations.RiShade.RiShadeManager.OnGameLeave, "RiShadeLeave");
		RunRuntimeAction(Xenostrap.Integrations.AntiAliasing.AntiAliasingManager.OnGameLeave, "AntiAliasingLeave");
		RunRuntimeAction(Xenostrap.Integrations.FrameGeneration.FrameGenManager.OnGameLeave, "FrameGenerationLeave");
	...
	private void EnsureRuntimeSessionWindows()
	{
		...
		if (Xenostrap.Integrations.Overlays.OverlayCrosshair.IsEnabled())
		{
			RunRuntimeAction(delegate
			{
				if (Xenostrap.Integrations.Overlays.OverlayHub.CompositorCrosshairActive)
				{
					return;
				}
	...
	private void ReconcileRuntimeSessionWindows()
	{
		...
		bool crosshairWanted = Xenostrap.Integrations.Overlays.OverlayCrosshair.IsEnabled()
			&& !Xenostrap.Integrations.Overlays.OverlayHub.CompositorCrosshairActive;
	...
	private static bool IsWindowManipulationEnabled()
	{
		...
		return !string.IsNullOrWhiteSpace(prop.RobloxTitle) && prop.RobloxTitle != "Xenostrap";
	}
