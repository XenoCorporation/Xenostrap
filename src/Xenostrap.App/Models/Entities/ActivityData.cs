...
namespace Xenostrap.Models.Entities;
...
public class ActivityData
{
	...
	public long UniverseId
	{
		...
		set
		{
			if (_universeId != value)
			{
				...
				{
					UniverseDetails = Xenostrap.Models.Entities.UniverseDetails.LoadFromCache(value);
				}
			}
		}
	}
	...
	public bool MachineAddressValid
	{
		get
		{
			return !string.IsNullOrWhiteSpace(MachineAddress) && !XenostrapMatchmaker.IsPrivateIp(MachineAddress);
		}
	}
	...
	public async Task<string?> QueryServerLocation(CancellationToken token = default)
	{
		...
		try
		{
			...
			(string Ip, int Port)? resolved = await XenostrapMatchmaker.ResolveJobServerAsync(PlaceId, JobId, timeout.Token).ConfigureAwait(false);
			if (resolved.HasValue && !XenostrapMatchmaker.IsPrivateIp(resolved.Value.Ip))
			{
				address = resolved.Value.Ip;
				MachineAddress = address;
			}
			if (string.IsNullOrWhiteSpace(address) || XenostrapMatchmaker.IsPrivateIp(address))
				return null;
			if (GlobalCache.TryGetServerLocation(address, out string? value))
			{
				return UnpackCachedLocation(value);
			}
			RobloxDatacenter? dc = await XenostrapMatchmaker.LookupUnknownIpAsync(address, timeout.Token).ConfigureAwait(false);
			...
			string iso = XenostrapMatchmaker.CountryToIso2(dc.Country);
			string country = XenostrapMatchmaker.CountryToDisplayName(dc.Country);
	...
	private void RejoinServer()
	{
		try
		{
			...
			ProcessStartInfo startInfo = new ProcessStartInfo
			{
				...
				CreateNoWindow = true,
				WorkingDirectory = System.IO.Path.GetDirectoryName(xenostrapPath) ?? ""
			};
