...
public partial class RobloxFullscreen
{
    ...
    public static async Task WaitAndTriggerFullscreenAsync(CancellationToken cancellationToken)
    {
        ...
        string processName = Xenostrap.App.RobloxPlayerAppName.Split('.')[0];
        Xenostrap.App.Logger.WriteLine(LOG_IDENT, $"Waiting for {processName} to start and become visible...");
        ...
        while (sw.Elapsed.TotalSeconds < 60 && !cancellationToken.IsCancellationRequested)
        {
            ...
            try
            {
                if (processes.Length > 0)
                {
                    ...
                    {
                        Xenostrap.App.Logger.WriteLine(LOG_IDENT, "Found visible Roblox window, triggering Alt+Enter");

                        SetForegroundWindow(roblox.MainWindowHandle);
                        await Task.Delay(500, cancellationToken).ConfigureAwait(false);

                        SendAltEnter();
                        Xenostrap.App.Logger.WriteLine(LOG_IDENT, "Alt+Enter triggered");
                        return;
                    }
                }
            }
        ...
        if (!cancellationToken.IsCancellationRequested)
            Xenostrap.App.Logger.WriteLine(LOG_IDENT, "Timed out waiting for Roblox window");
    }
