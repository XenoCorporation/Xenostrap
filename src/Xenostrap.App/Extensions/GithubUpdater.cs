...
public static class GithubUpdater
{
    ...
    {
        HttpClient client = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromMinutes(10));
        client.DefaultRequestHeaders.TryAddWithoutValidation("User-Agent", "Xenostrap-Updater");
        return client;
    }
    ...
    public static async Task<string?> GetLatestVersionTagAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                var releases = await Xenostrap.Utility.GitHubCache.GetJsonWithFallbackAsync<List<Xenostrap.Models.APIs.GitHub.GithubRelease>>(
            ...
            string? response = await Xenostrap.Utility.GitHubCache.GetStringWithFallbackAsync(
    ...
    public static async Task<bool> DownloadAndInstallUpdate(string tag, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (release == null)
            {
                string? response = await Xenostrap.Utility.GitHubCache.GetStringWithFallbackAsync(App.ProjectReleaseApi, App.ProjectFallbackReleaseApi, TimeSpan.FromMinutes(15));
                cancellationToken.ThrowIfCancellationRequested();
                if (response == null)
                    return false;
                using var doc = JsonDocument.Parse(response);
                ...
                release = new Xenostrap.Models.APIs.GitHub.GithubRelease
                {
                    TagName = latestTag,
            ...
            App.Logger.WriteLine("GitHubUpdater", OperatingSystem.IsLinux() ? "No valid Linux bundle asset found." : "No valid Xenostrap executable asset found.");
            return false;
        }
        catch (OperationCanceledException)
        {
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        using Xenostrap.Utility.InterProcessLock updateLock = new("AutoUpdater", TimeSpan.FromSeconds(5));
        if (!updateLock.IsAcquired)
            throw new IOException("Another update is already in progress");
        cancellationToken.ThrowIfCancellationRequested();

        string tempDirectory = Path.Combine(Path.GetTempPath(), "Xenostrap_Update_" + Guid.NewGuid().ToString("N"));
    ...
    private static async Task<bool> UpdateExe(string url, string name, string digest, string tag, CancellationToken cancellationToken)
    {
        ...
        if (!updateLock.IsAcquired)
            throw new IOException("Another update is already in progress");
        cancellationToken.ThrowIfCancellationRequested();
        string tempDir = Path.Combine(Path.GetTempPath(), "Xenostrap_Update");
    ...
    private static void UpdateInstalledMetadata(string tag)
    {
        ...
        try
        {
            string? installedRoot = Xenostrap.Utility.InstallRecord.Read();
            ...
            if (version.Length == 0 || version.Length > 64 || version.Any(char.IsControl))
                return;
            using RegistryKey? key = Registry.CurrentUser.OpenSubKey("Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Xenostrap", true);
    ...
    private static async Task DownloadToFileAsync(string url, string path, string digest, CancellationToken token)
    {
        ...
        await Xenostrap.Utility.ResilientDownload.DownloadAsync(http, [url], path, MaxUpdateBytes, token, digest).ConfigureAwait(false);
    }
}
