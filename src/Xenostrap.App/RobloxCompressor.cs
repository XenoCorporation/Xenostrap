...
{
    private const string ManifestFileName = "XenostrapCompressed.json";
    ...
    public static async Task RepairCorruptedAsync()
    {
        ...
        foreach (string dir in Directory.GetDirectories(versionsDir))
        {
            ...
            try
            {
                ...
                foreach (string pkgManifestUrl in Xenostrap.RobloxInterfaces.Deployment.GetLocations($"/{versionGuid}-rbxPkgManifest.txt"))
                {
                    ...
                    try
                    {
                        ...
                        manifestText = await Xenostrap.Utility.Http.ReadStringBoundedAsync(resp.Content, 8388608, CancellationToken.None).ConfigureAwait(false);
                        break;
                    }
    ...
    private static ManifestData LoadManifest(string versionDir)
    {
        ...
        try
        {
            return Xenostrap.Utility.JsonFile.Deserialize<ManifestData>(path, Xenostrap.Utility.JsonOptions.Tolerant, 16777216);
        }
        catch
        {
    ...
    {
        string path = System.IO.Path.Combine(versionDir, ManifestFileName);
		Xenostrap.Utility.JsonFile.SerializeAtomic(path, manifest);
    }
