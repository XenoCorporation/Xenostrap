...
internal static class Paths
{
	...
	public static string Temp => Path.Combine(Path.GetTempPath(), "Xenostrap");
	...
	public static string DocumentsData
	{
		get
		{
			...
			return string.IsNullOrWhiteSpace(documents) ? Path.Combine(UserData, "Account") : Path.Combine(documents, "Xenostrap");
		}
	}

	private static string ResolveUserData()
	{
		string preferred = Path.Combine(LocalAppData, "Xenostrap");
		try
		{
	...
	private static void MigrateLegacyUserData(string target)
	{
		try
		{
			...
			if (string.IsNullOrWhiteSpace(documents))
				return;
			string legacy = Path.Combine(documents, "Xenostrap");
	...
	public static string RobloxBase { get; set; } = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Xenostrap");
	...
	public static void Initialize(string baseDirectory, string? robloxBaseDirectory = null)
	{
		...
		Base = baseDirectory;
		CloudSynced = Xenostrap.Installer.IsCloudSyncedPath(baseDirectory);
		string offloadRoot = (robloxBaseDirectory ?? (CloudSynced ? Path.Combine(LocalAppData, "Xenostrap") : baseDirectory));
		RobloxBase = offloadRoot;
		Application = Path.Combine(Base, Xenostrap.Utility.Platform.IsWindows ? "Xenostrap.exe" : "Xenostrap");
		...
		Mods = Path.Combine(Base, "XenostrapMods");
		RobloxClients = Path.Combine(offloadRoot, "RobloxClients");
		WebViewData = Path.Combine(LocalAppData, "Xenostrap", "WebView2");
		InitializeDerivedPaths();
	}

	public static void InitializePortable(Xenostrap.Platform.PlatformStoragePaths storage, string applicationPath)
	{
		if (storage == null)
		{
		...
		Versions = Path.Combine(RobloxBase, "RblxVersions");
		Mods = Path.Combine(Data, "XenostrapMods");
	...
	private const string DataFormatVersion = "0.1.0.0";
	...
	private static void ResetLegacyUserData(string root)
	{
		...
		try
		{
			userData = Path.GetFullPath(Path.Combine(LocalAppData, "Xenostrap")).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
		}
		catch
		{
			return;
	...
	private static void RemoveLegacyDocumentsData()
	{
		try
		{
			...
			string legacy = Path.GetFullPath(Path.Combine(documentsRoot, "Xenostrap")).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
	...
	public static void ResetUserData()
	{
		...
		{
			throw new InvalidOperationException("The Xenostrap data location is unsafe to reset");
		}
	...
	private static void DeleteOwnedDirectory(string path, string basePath)
	{
		...
		{
			throw new InvalidOperationException("A Xenostrap data location is outside the expected folders");
		}
		...
		throw new IOException("Xenostrap data could not be removed from " + fullPath, failure);
	}
