...
public static class ServerFetchStore
{
	...
	public const string OfficialPresetUrl = "https://github.com/XenoCorporation/Xenostrap/blob/main/assets/Datacenters/ServerFetch.json";
	public const string FallbackPresetUrl = "https://github.com/XenoCorporation/Xenostrap/blob/main/assets/Datacenters/ServerFetch.json";
	...
	private static readonly JsonSerializerOptions StoreJsonOptions = new(Xenostrap.Utility.JsonOptions.Tolerant)
	{
		WriteIndented = true,
	...
	{
		HttpClient client = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromSeconds(15L));
		client.DefaultRequestHeaders.UserAgent.ParseAdd("Xenostrap/1.0");
	...
	private static void PrunePrivateCidrs()
	{
		try
		{
			...
			foreach (KeyValuePair<string, LearnedServerEntry> server in _data.Servers)
			{
				if (XenostrapMatchmaker.IsPrivateIp(server.Key.Split('/')[0]))
				{
					list.Add(server.Key);
				}
			}
	...
	private static void Load()
	{
		try
		{
			...
			if (!Xenostrap.Utility.JsonFile.TryLoad<ServerFetchData>(FilePath, StoreJsonOptions, out ServerFetchData? serverFetchData, out bool recovered, out Exception? failure, MaxStoreBytes) || serverFetchData == null)
		...
		try
		{
			...
			lock (_lock)
			{
				foreach (KeyValuePair<string, LearnedServerEntry> server in serverFetchData.Servers)
				{
					...
					if (string.IsNullOrWhiteSpace(text) || XenostrapMatchmaker.IsPrivateIp(text.Split('/')[0]))
					{
						continue;
					}
					string text2 = server.Value.City ?? "";
					string text3 = server.Value.Region ?? "";
					string text4 = XenostrapMatchmaker.NormalizeCountryCode(server.Value.Country ?? "");
	...
	{
		if (string.IsNullOrWhiteSpace(ip) || XenostrapMatchmaker.IsPrivateIp(ip))
		{
			return;
		}
		EnsureLoaded();
		...
		SaveThrottled();
		Xenostrap.Utility.WebsiteGeoSync.PushSoon();
	}
	...
	public static List<LearnedServerEntry> SortedByDistance(double lat, double lon, int max = 10)
	{
		...
		lock (_lock)
		{
			return (from e in _data.Servers.Values
				...
				orderby XenostrapMatchmaker.HaversineKm(lat, lon, e.Lat, e.Lon)
	...
	public static void SaveNow()
	{
		...
		try
		{
			...
			Directory.CreateDirectory(FolderPath);
			Xenostrap.Utility.JsonFile.WriteAtomicText(FilePath, contents);
		}
		catch (Exception ex)
		{
