...
public static class BootstrapperThemes
{
    ...
    {
        HttpClient client = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromMinutes(3));
        client.MaxResponseContentBufferSize = MaxArchiveBytes;
        client.DefaultRequestHeaders.UserAgent.ParseAdd("Xenostrap/" + App.Version);
        return client;
    }
    ...
    private static bool ConfirmActiveContent(string name, bool verified, List<string> activeFiles)
    {
        ...
        System.Windows.MessageBoxResult result = Xenostrap.UI.Frontend.ShowMessageBox(
            message,
    ...
    public static ThemePublishRecord? ReadInstallRecord(string themeFolderName)
    {
        try
        {
			...
			return Xenostrap.Utility.JsonFile.Deserialize<ThemePublishRecord>(path, Xenostrap.Utility.JsonOptions.Tolerant, 1048576);
        }
        catch
        {
            return null;
    ...
    private static void WriteInstallRecord(string themeFolderName, ThemePublishRecord record)
    {
        ...
        {
            string path = Path.Combine(Paths.CustomThemes, themeFolderName, InstallRecordFile);
			Xenostrap.Utility.JsonFile.SerializeAtomic(path, record, Xenostrap.Utility.JsonOptions.Indented);
        }
        catch (Exception ex)
        {
    ...
    private static void WritePublishRecord(string themeFolderName, ThemePublishRecord record)
    {
        ...
        {
            string path = Path.Combine(Paths.CustomThemes, themeFolderName, PublishRecordFile);
			Xenostrap.Utility.JsonFile.SerializeAtomic(path, record);
        }
        catch (Exception ex)
        {
    ...
    private static string RequireToken()
    {
        ...
        if (string.IsNullOrWhiteSpace(authToken))
            throw new InvalidOperationException("Sign in to your Xenostrap account first, from the Home page");
