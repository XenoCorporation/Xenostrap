...
namespace Xenostrap.Integrations.Rojo
{
    public static class RojoManager
    {
        ...
        private static async Task<(string Tag, string Url, string Digest, long Size)> ResolveLatestAsync(CancellationToken ct)
        {
            ...
            string metadata = await Xenostrap.Utility.Http.ReadStringBoundedAsync(response.Content, MaxReleaseMetadataBytes, ct).ConfigureAwait(false);
        ...
        {
            await Xenostrap.Utility.ResilientDownload.DownloadAsync(Http, [url], outputPath, size, ct, digest,
                progress: (read, total) =>
                {
