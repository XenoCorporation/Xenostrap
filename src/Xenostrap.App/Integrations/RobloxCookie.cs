...
public static class RobloxCookie
{
	...
	private static readonly HttpClient _authClient = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromSeconds(12L), handler =>
	{
	...
	public static async Task<RobloxAccount?> GetAccountAsync(string cookie, CancellationToken ct = default(CancellationToken))
	{
		...
		try
		{
			...
			req.Headers.TryAddWithoutValidation("User-Agent", "Xenostrap/1.0");
