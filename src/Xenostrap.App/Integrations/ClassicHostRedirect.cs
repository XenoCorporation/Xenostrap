...
public static class ClassicHostRedirect
{
	...
	private const string Marker = "# XENOSTRAP-ORC";
			...
			if (ProcessElevation.IsAdministrator())
			{
				if (!enable)
					return await Task.Run(Remove, cancellationToken).ConfigureAwait(false)
						? null
						: "Xenostrap could not restore normal Roblox name resolution.";
				if (!await Task.Run(Apply, cancellationToken).ConfigureAwait(false))
					return "Xenostrap could not point the classic Roblox domains at the local server. Check that the Windows hosts file is writable.";
	...
	private static void RemoveStale(bool allowElevation)
	{
		try
		{
			...
			if (Xenostrap.Utility.ClassicClients.AnyClassicClientRunning())
			{
			...
			if (!allowElevation)
			{
				App.Logger?.WriteLine(LOG_IDENT, "A previous classic session left the redirect behind, it will be cleared on the next Xenostrap start");
				return;
			}
		...
		try
		{
			...
			if (started)
			{
				...
				while (idle < 6)
				{
					if (owner != null && owner.HasExited)
						break;
					idle = Xenostrap.Utility.ClassicClients.AnyClassicClientRunning() ? 0 : idle + 1;
					Thread.Sleep(500);
				}
			}
			else
	...
	private static async Task<string?> RunElevatedAsync(bool enable, CancellationToken cancellationToken)
	{
		...
		{
			return "Xenostrap could not locate its own executable to request elevation.";
		}
		try
		{
			...
			if (process == null)
			{
				return "Windows did not start the elevated Xenostrap helper.";
			}
			if (enable)
			{
				for (int i = 0; i < 60; i++)
				...
				if (!IsApplied())
					return "Xenostrap could not point the classic Roblox domains at the local server. Check that the Windows hosts file is writable.";
				if (!IsUrlAclReserved())
					return "Xenostrap could not reserve port 80 for the local ORC server. Check that no other program owns " + ServerPrefix + ".";
				return await WaitForLoopbackResolutionAsync(cancellationToken).ConfigureAwait(false);
			}
		...
		catch (Win32Exception ex) when (ex.NativeErrorCode == 1223)
		{
			...
			return "Xenostrap needs administrator rights to point the classic Roblox domains at the local server. The elevation prompt was declined.";
		}
		catch (OperationCanceledException)
		{
			...
			return "The elevated Xenostrap helper did not finish in time.";
		}
		catch (Exception ex)
		{
			App.Logger?.WriteLine(LOG_IDENT, "Elevated helper failed: " + ex.Message);
			return "The elevated Xenostrap helper failed: " + ex.Message;
		}
		if (IsApplied() != enable)
		{
			return enable
				? "Xenostrap could not point the classic Roblox domains at the local server. Check that the Windows hosts file is writable."
				: "Xenostrap could not restore normal Roblox name resolution.";
		}
		return null;
	}
