...
using Xenostrap.UI;
using MatchmakerAttempt = Xenostrap.Models.Persistable.MatchmakerAttempt;
...
public sealed class ServerMatchmaker : IDisposable
{
	...
	private async Task HandleGameJoinAsync()
	{
		...
		try
		{
			...
			if (!App.Settings.Prop.XenostrapMatchmakerEnabled && !HasPerGamePreference(data.PlaceId))
			{
	...
	private static async Task RecordLearningAsync(ActivityData data)
	{
		try
		{
			...
			RobloxDatacenter? dc = await XenostrapMatchmaker.LookupUnknownIpAsync(data.MachineAddress, timeoutCts.Token).ConfigureAwait(false);
			if (dc != null)
			{
	...
	public static string ResolvePreferredDatacenterKey(long placeId)
	{
		...
		return (App.Settings.Prop.XenostrapMatchmakerPreferredDatacenter ?? "").Trim();
	}
	...
	private async Task EvaluateAsync(ActivityData data, CancellationToken token)
	{
		...
		UserGeo? geo = await XenostrapMatchmaker.GetUserGeoAsync(token).ConfigureAwait(false);
		if (geo == null)
		{
		...
		RobloxDatacenter? currentDc = RobloxDatacenterMap.Map(data.MachineAddress)
			?? await XenostrapMatchmaker.LookupUnknownIpAsync(data.MachineAddress, token).ConfigureAwait(false);
		if (currentDc == null)
		{
		...
		double currentKm = XenostrapMatchmaker.HaversineKm(geo.Lat, geo.Lon, currentDc.Lat, currentDc.Lon);
		int currentPing = XenostrapMatchmaker.EstimatePingMs(currentKm);
		string currentKey = XenostrapMatchmaker.DatacenterKey(currentDc);
		bool currentIsBlocked = XenostrapMatchmaker.GetBlockedDatacenters().Contains(currentKey);

		string preferredKey = ResolvePreferredDatacenterKey(data.PlaceId);
		bool wantsOtherDc = preferredKey.Length > 0 && !XenostrapMatchmaker.MatchesPreferredDc(currentDc, preferredKey);
		...
		MatchmakerCandidate? best = await XenostrapMatchmaker
			.PickBestJobIdAsync(data.PlaceId, tried, XenostrapMatchmaker.ResolveEffectiveCandidateCount(), token, preferredKey)
		...
		bool sameDc = string.Equals(currentKey, XenostrapMatchmaker.DatacenterKey(best.Datacenter), StringComparison.OrdinalIgnoreCase);
		bool bestIsPreferred = preferredKey.Length > 0 && XenostrapMatchmaker.MatchesPreferredDc(best.Datacenter, preferredKey);
	...
	private async Task TriggerRejoinAsync(long placeId, int attemptNumber, CancellationToken token, string? explicitJobId, string? targetName)
	{
		...
		try
		{
			...
			if (!string.IsNullOrEmpty(process))
			{
				...
				if (spawnedSuccessor)
					App.Logger.WriteLine(LOG_IDENT, "Spawned successor Xenostrap");
			}
		}
		catch (Exception ex)
		{
			App.Logger.WriteLine(LOG_IDENT, "Could not spawn successor Xenostrap: " + ex.Message);
		}
	...
	private void ShowAlert(string text, int durationSeconds)
	{
		try
		{
			...
			if (icon != null && icon.EnableAppNotifications)
				icon.ShowAlert("Xenostrap Matchmaker", text, durationSeconds, null);
		}
		catch (Exception ex)
		{
