...
using Xenostrap.Core.Networking;
using ZstdSharp;
...
public static class AssetProxyServer
{
	public static bool IsRequired =>
		Xenostrap.Utility.Platform.IsWindows &&
	...
	public static bool UsesExplicitProxy => Xenostrap.Utility.Platform.IsLinux;
	...
	{
		if (App.LaunchSettings?.RobloxLaunchMode != Xenostrap.Enums.LaunchMode.Player)
			return;
		if (!IsRequired)
		{
		...
		try
		{
			...
			if (UsesExplicitProxy)
			{
				...
				Xenostrap.Platform.OperationResult enabled = await LinuxAssetWarpBridge.EnableAsync(address, linked.Token).ConfigureAwait(false);
	...
	{
		{
			...
			try
			{
				while (!ct.IsCancellationRequested)
				{
					...
					if (upstream != null && Volatile.Read(ref _upstreamNetworkVersion) != Xenostrap.Utility.VpnHttpClient.NetworkVersion)
					{
						upstream.Dispose();
						upstream = null;
						remote = null;
	...
	private static async Task HandleExplicitClientAsync(TcpClient client, CancellationToken ct)
	{
		using (client)
		{
			...
			if (!Xenostrap.Core.AssetProxy.ProxyRequestTarget.TryResolve(parts[1], request.Get("Host"), isConnect, out string host, out int port))
			{
