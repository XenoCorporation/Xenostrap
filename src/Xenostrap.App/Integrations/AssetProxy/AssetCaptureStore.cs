...
public static partial class AssetCaptureStore
{
	...
	private static readonly HttpClient _robloxClient = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromSeconds(25L), handler =>
	{
		...
		try
		{
			lock (_indexLock)
			{
				if (force || !((DateTime.UtcNow - _lastIndexWrite).TotalSeconds < 30.0))
				{
					...
					Xenostrap.Utility.JsonFile.SerializeAtomic(path, _index);
				}
			}
		}
		catch
	...
	private static void LoadIndex()
	{
		try
		{
			...
			Dictionary<string, IndexRecord> dictionary = Xenostrap.Utility.JsonFile.Deserialize<Dictionary<string, IndexRecord>>(path, Xenostrap.Utility.JsonOptions.Tolerant, MaxIndexBytes);
