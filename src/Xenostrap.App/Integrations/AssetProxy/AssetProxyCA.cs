...
internal static partial class AssetProxyCA
{
	...
	private const string CA_SUBJECT = "CN=Xenostrap Proxy CA";
	...
	private const string CaPassword = "XenostrapProxyCA";

	private static readonly byte[] CaProtectionEntropy = Encoding.UTF8.GetBytes("Xenostrap AssetWarp CA");
	...
	private static void PatchBundle(string path, string caPem)
	{
		...
		foreach (Match match in matches)
		{
			...
			if (!IsXenostrapCertificate(match.Value))
			{
		...
		try
		{
			string temporary = path + ".xenostrap.tmp";
	...
	private static bool IsXenostrapCertificate(string pem)
	{
		try
		{
	...
	private static void RemoveFromBundle(string path, string? keepThumbprint)
	{
		...
		{
			cleaned.Append(original, cursor, match.Index - cursor);
			string? thumbprint = GetXenostrapCertificateThumbprint(match.Value);
	...
	private static string? GetXenostrapCertificateThumbprint(string pem)
	{
		try
		{
			using X509Certificate2 certificate = X509Certificate2.CreateFromPem(pem);
