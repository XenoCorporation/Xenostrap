...
public static class RobloxAuthLauncher
{
	...
	private static readonly HttpClient ServerClient = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromSeconds(10L));
	private static readonly HttpClient AuthClient = CreateAuthClient();

	private static HttpClient CreateAuthClient()
	{
		HttpClient client = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromSeconds(15L), handler =>
		{
