...
using Xenostrap.Models.XenostrapRPC;
...
public class DiscordRichPresence : IDisposable
{
	...
	private const string IdleDetails = "Inside Xenostrap";

	private const string IdleState = "Browsing Roblox";

	private static readonly string? LaunchStatusFile = Environment.GetEnvironmentVariable("XENOSTRAP_STATUS_FILE");
	...
	private readonly DiscordRpcClient _rpcClient = new DiscordRpcClient("1539279631264518205");
	...
	public void ProcessRPCMessage(Message message, bool implicitUpdate = true)
	{
		...
		{
			if (!TryDeserializePresence(message.Data, out Xenostrap.Models.XenostrapRPC.RichPresence presence) || presence == null)
			{
				return;
			}
	...
	private static bool TryDeserializePresence(JsonElement data, out Xenostrap.Models.XenostrapRPC.RichPresence? presence)
	{
		try
		{
			presence = data.Deserialize<Xenostrap.Models.XenostrapRPC.RichPresence>();
	...
	private static async Task<string> GetPlaceNameAsync(long placeId, CancellationToken token)
	{
		...
		try
		{
			...
			request.Headers.TryAddWithoutValidation("User-Agent", "Xenostrap/1.0");
	...
	private async Task SetIdlePresenceAsync()
	{
		try
		{
			...
			string smallImage = "xenostrap";
			string smallText = "Xenostrap";
			try
			{
			...
			{
				_currentPresence.Details = "Inside Xenostrap";
			...
			_lastPresenceSignature = signature;
			UpdatePresence(force: true);
			App.Logger.WriteLine("DiscordRichPresence", "Set idle presence (Inside Xenostrap).");
		}
		catch (Exception ex)
		{
	...
	private async Task<(string key, string text)> GetSmallImageAsync(ActivityData activity)
	{
		...
		{
			return (key: "xenostrap", text: "Xenostrap");
		}
		try
		{
			...
			return (key: userDetails.Thumbnail?.ImageUrl ?? "xenostrap", text: userDetails.Data.DisplayName + " (@" + userDetails.Data.Name + ")");
		}
		catch
		{
			return (key: "xenostrap", text: "Xenostrap");
	...
	public Xenostrap.Models.PresenceSnapshot GetPresenceSnapshot()
	{
		DiscordRPC.RichPresence? presence = _currentPresence;
		if (_disposed || presence == null)
		{
			return new Xenostrap.Models.PresenceSnapshot { Connected = !_disposed && _rpcClient.IsInitialized, Active = false };
		}
		Xenostrap.Models.PresenceSnapshot snapshot = new Xenostrap.Models.PresenceSnapshot
		{
		...
		foreach (Button button in buttons)
		{
			...
			snapshot.Buttons.Add(new Xenostrap.Models.PresenceButton { Label = button.Label ?? string.Empty, Url = button.Url ?? string.Empty });
		}
		return snapshot;
	}
