...
public static class StudioPluginInstaller
{
	...
	public static void EnsureInstalled(bool force = false)
	{
		try
		{
			...
			if (flag)
			{
				...
				App.Logger.WriteLine(LogTag, "Xenostrap Studio plugin written to " + PluginFile);
			}
			InstallIcon();
		}
		catch (Exception ex)
			...
			foreach (string item in list)
			{
				...
				foreach (string path in directories)
				{
					try
					{
						...
						{
							string text2 = Path.Combine(text, "XenostrapStudio.png");
	...
	private static byte[]? ReadAppLogo()
	{
		try
		{
			StreamResourceInfo resourceStream = Application.GetResourceStream(new Uri("pack://application:,,,/Xenostrap.png"));
	...
	private static readonly string[] ModernOnlyPlugins = new string[2] { "XenostrapStudio.lua", "RojoManagedPlugin.rbxm" };
