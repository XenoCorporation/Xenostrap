...
namespace Xenostrap.Integrations.FrameGeneration
{
    ...
    internal sealed class NvFrucEngine : IDisposable
    {
        ...
        private const string InstallBaseUrl = "https://xenostrapp.pages.dev/assets/bin/";
        ...
        private static void InstallWorker()
        {
            try
            {
                ...
                App.Logger.WriteLine("NvFruc", "Downloading the NVIDIA interpolation components from the Xenostrap site, one time only");
        ...
        private static bool DownloadOne(string name, long size, string sha256, string dir)
        {
            ...
            try
            {
                Xenostrap.Utility.ResilientDownload.DownloadAsync(App.HttpClient, [InstallBaseUrl + name], path, size, cts.Token, sha256).GetAwaiter().GetResult();
            }
