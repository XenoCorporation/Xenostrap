...
using Xenostrap.Integrations.Overlays;
...
namespace Xenostrap.Integrations.GameChat
{
    public class GameChatProfileWindow : Window
    {
        ...
        {
            var data = await GameChatRoblox.GetXenostrapProfileAsync(_robloxId, _lifetimeCts.Token);
			if (_closed || _lifetimeCts.IsCancellationRequested)
				return;
            if (data == null || !data.Exists)
            {
                _displayName.Text = "No Xenostrap profile";
                _username.Text = "";
                _about.Text = "This user has not set up a Xenostrap profile.";
        ...
        private (ImageSource? Avatar, BorderRender? Border) ResolveVisuals(GameChatXenostrapProfile data)
        {
        ...
        private void ApplyGradient(GameChatXenostrapProfile data)
        {
        ...
        private void ApplyFriendState(GameChatXenostrapProfile data)
        {
            if (data.Self)
            {
        ...
        private async void OnAddFriendClick(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                var (ok, error) = await GameChatRoblox.AddXenostrapFriendAsync(_robloxId, _lifetimeCts.Token);
				if (_closed)
					return;
                if (ok)
                {
