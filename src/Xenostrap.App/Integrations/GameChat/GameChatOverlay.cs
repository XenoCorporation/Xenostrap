        ...
        private async Task HandleUpdateCheck()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(tag) && tag.TrimStart('v') != current.TrimStart('v'))
                {
                    ...
                    OpenUrl(release?.Assets?.FirstOrDefault(asset => string.Equals(asset.Name, "Xenostrap.exe", StringComparison.OrdinalIgnoreCase))?.BrowserDownloadUrl ?? App.ProjectFallbackDownloadLink);
                }
                else
                {
