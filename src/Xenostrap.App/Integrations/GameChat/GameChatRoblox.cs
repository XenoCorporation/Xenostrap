...
{
    public sealed class GameChatXenostrapProfile
    {
    ...
    public static class GameChatRoblox
    {
        ...
        private static readonly HttpClient _authClient = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromSeconds(12), handler =>
        {
        ...
        private static async Task<GameChatIdentity?> FetchChatIdentityWithTimeoutAsync(long accountId)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    GameChatXenostrapProfile? profile = await GetXenostrapProfileAsync(accountId, timeout.Token).ConfigureAwait(false);
        ...
        public static async Task<GameChatXenostrapProfile?> GetXenostrapProfileAsync(long robloxId, CancellationToken token = default)
        {
            if (robloxId <= 0)
                return null;
            try
            {
                ...
                var profile = new GameChatXenostrapProfile { RobloxId = robloxId < LocalAccountIdBase ? robloxId : 0 };
            ...
            {
                App.Logger.WriteLine(Tag, "Xenostrap profile fetch failed: " + ex.Message);
                return null;
            }
        }

        public static async Task<(bool Ok, string? Error)> AddXenostrapFriendAsync(long robloxId, CancellationToken token = default)
        {
            if (robloxId <= 0)
