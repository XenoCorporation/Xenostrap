...
namespace Xenostrap.Integrations.AntiAliasing
{
    ...
    {
        private const string ClassName = "XenostrapAntiAliasingOverlay";
        ...
        private bool UpdateVisibility(CancellationToken token)
        {
			...
			bool robloxActive = (_robloxHwnd != IntPtr.Zero && fg == _robloxHwnd) || (fg != IntPtr.Zero && fg == Xenostrap.Integrations.RiShade.RiShadePanel.CurrentHwnd) || OverlayDiagnostics.IsOverlayHandle(fg);
            if (robloxActive)
            {
                if (_hiddenByFocus)
                {
