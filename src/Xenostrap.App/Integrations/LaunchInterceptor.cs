...
public static class LaunchInterceptor
{
	...
	public static async Task<string?> MaybeRewriteForClosestAsync(string? robloxLaunchArgs, CancellationToken token = default(CancellationToken))
	{
		...
		try
		{
			...
			if (!App.Settings.Prop.XenostrapMatchmakerEnabled && !ServerMatchmaker.HasPerGamePreference(placeId))
			{
			...
			MatchmakerCandidate matchmakerCandidate = await XenostrapMatchmaker.PickBestJobIdAsync(placeId, null, XenostrapMatchmaker.ResolveEffectiveCandidateCount(), token, preferredKey).ConfigureAwait(continueOnCapturedContext: false);
