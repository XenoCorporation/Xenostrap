...
using Xenostrap.Integrations.AntiAliasing;
using Xenostrap.Integrations.FrameGeneration;

namespace Xenostrap.Integrations.Overlays
{
    public static class OverlayDiagnostics
    {
        ...
        public static string BuildReport()
        {
            ...
            sb.AppendLine("Xenostrap overlay diagnostics");
            sb.AppendLine();
            ...
            if (likelyRobloxFullscreen)
            {
                ...
                sb.AppendLine("   Windows gives an exclusive fullscreen game the whole screen and will not draw ANY external overlay on top of it, including Xenostrap's. This is the number one reason overlays vanish once you are in first person.");
                sb.AppendLine("   FIX: In Roblox, press Esc, open Settings, set Display Mode to Windowed. Or enable Fake Borderless Fullscreen in Xenostrap so Roblox runs borderless (overlays can draw over borderless, never over exclusive fullscreen).");
                sb.AppendLine();
            }
            ...
            if (fakeExclusive)
            {
                ...
                sb.AppendLine("   It presents Roblox through a black fullscreen backdrop window that sits on top of everything. Xenostrap now re raises the overlays above that backdrop automatically, but if an overlay still does not show, this backdrop is the cause.");
            ...
            if (n == 1)
            {
                ...
                sb.AppendLine("If you still cannot see them, Roblox is almost certainly running in its own exclusive Fullscreen display mode. Press Esc in Roblox, open Settings and set Display Mode to Windowed, or enable Fake Borderless Fullscreen in Xenostrap.");
            }
