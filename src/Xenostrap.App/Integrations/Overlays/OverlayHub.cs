...
namespace Xenostrap.Integrations.Overlays
{
    public static class OverlayHub
    {
				...
				app.Dispatcher.BeginInvoke(new Action(() =>
				{
					try
					{
						if (!OverlayCrosshair.IsEnabled() || _compositorLive)
							return;
						if (app.Resources["CrosshairWindow"] is Xenostrap.UI.Elements.Crosshair.CrosshairWindow)
							return;
						app.Resources["CrosshairWindow"] = new Xenostrap.UI.Elements.Crosshair.CrosshairWindow(new Xenostrap.UI.ViewModels.Settings.ModsViewModel());
        ...
        private static void Supervise(CancellationTokenSource owner)
        {
            ...
            try
            {
                mutex = new Mutex(false, "XenostrapOverlayCompositorActive");
                    ...
                    if (!held)
                    {
                        ...
                        if (!held)
                        {
                            if (!loggedWaitOwner)
                            {
                                ...
                                App.Logger.WriteLine("Overlays", "Another Xenostrap process is running the compositor, standing by");
                            }
