    ...
    public static class RobloxFpsCap
    {
        ...
        private static string BaseFpsPath()
        {
            string root = !string.IsNullOrEmpty(Paths.Config)
                ? Paths.Config
                : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Xenostrap", "Config");
        ...
        private static void Reload()
        {
            try
            {
                ...
                if (m.Success && int.TryParse(m.Groups[1].Value, out int v))
                {
                    ...
                    if (Xenostrap.Integrations.FrameGeneration.FrameGenSettings.ModeIndex > 0 && preferred >= 5 && preferred < 1000)
                        v = preferred;
                    if (v >= 5 && v < 15)
                        v = 15;
