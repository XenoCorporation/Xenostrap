...
public class QuestTracker : IDisposable
{
    ...
    public Func<Xenostrap.UI.NotifyIconWrapper?>? NotifyIconResolver { get; set; }

    public static Xenostrap.Models.QuestProgressSnapshot? Progress { get; private set; }
    ...
    private static Xenostrap.Models.QuestProgressSnapshot? BuildSnapshot(string body, long universeId)
    {
        try
        {
            ...
            if (!root.TryGetProperty("quests", out JsonElement quests) || quests.ValueKind != JsonValueKind.Array)
                return null;
            Xenostrap.Models.QuestLiveSession? live = Xenostrap.Models.QuestLiveSession.FromResponse(root);
            Xenostrap.Models.QuestProgressSnapshot snapshot = new Xenostrap.Models.QuestProgressSnapshot { UniverseId = universeId, Session = live };
    ...
    private static void AppendQuestLines(
        Xenostrap.Models.QuestProgressSnapshot snapshot,
        JsonElement quests,
        long universeId,
        Xenostrap.Models.QuestLiveSession? live)
    {
        foreach (JsonElement quest in quests.EnumerateArray())
        {
            ...
            if (!relevant)
                continue;
            snapshot.Lines.Add(new Xenostrap.Models.QuestProgressLine
            {
