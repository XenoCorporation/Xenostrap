...
{
	private const string GameMessageEntry = "[FLog::Output] [XenostrapRPC]";
	...
	private const string GameMessageEntryPattern = "\\[XenostrapRPC\\] (.*)";
	...
	private static readonly Regex LogPattern6 = new Regex("\\[XenostrapRPC\\] (.*)", RegexOptions.Compiled | RegexOptions.CultureInvariant);
	...
	public async Task<string?> GetGameIconAsync()
	{
		try
		{
			...
			using JsonDocument jsonDocument = JsonDocument.Parse(await Xenostrap.Utility.Http.GetString(requestUri).ConfigureAwait(false));
	...
	public async Task<int> GetGameTotalPlayingAsync()
	{
		...
		try
		{
			...
			using JsonDocument jsonDocument = JsonDocument.Parse(await Xenostrap.Utility.Http.GetString(requestUri, cts.Token).ConfigureAwait(continueOnCapturedContext: false));
	...
	public async Task<ServerInfo?> GetCurrentServerInfoAsync()
	{
		...
		for (int pages = 0; pages < 6; pages++)
		{
			...
			try
			{
				serverListResponse = JsonSerializer.Deserialize<ServerListResponse>(await Xenostrap.Utility.Http.GetString(text2, cts.Token).ConfigureAwait(continueOnCapturedContext: false), JsonOptions.CaseInsensitive);
			}
			catch
			{
				break;
	...
	private void ReadLogEntry(string entry)
	{
		...
		if (!InGame && Data.PlaceId == 0L)
		{
			...
			if (match.Groups.Count == 4)
			{
				...
				Data.MachineAddress = XenostrapMatchmaker.IsPrivateIp(joinAddress) ? string.Empty : joinAddress;
				if (_teleportMarker)
				{
		...
		else if (!InGame && Data.PlaceId != 0L)
		{
			...
			else if (entry.Contains("[FLog::Network] UDMUX Address = "))
			{
				...
				if (match4.Groups.Count == 3)
				{
					...
					string routable = !XenostrapMatchmaker.IsPrivateIp(udmuxAddress)
						? udmuxAddress
						: (!XenostrapMatchmaker.IsPrivateIp(rccAddress) ? rccAddress : string.Empty);
		...
		else
		{
			...
			else if (entry.Contains("[FLog::Output] [XenostrapRPC]"))
			{
		...
		try
		{
			...
			try
			{
				...
				if (!response.IsSuccessStatusCode)
					return;
				using JsonDocument document = JsonDocument.Parse(await Xenostrap.Utility.Http.ReadStringBoundedAsync(response.Content, 1048576, timeout.Token).ConfigureAwait(false));
