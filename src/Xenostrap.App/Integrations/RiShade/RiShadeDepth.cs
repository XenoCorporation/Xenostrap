...
namespace Xenostrap.Integrations.RiShade
{
    public static class RiShadeDepth
    {
        ...
        private const string DirectMlUrl = "https://xenostrapp.pages.dev/assets/bin/DirectML.dll";
        ...
        private static bool EnsureRuntimeFiles(CancellationToken token)
        {
            try
            {
                if (!FileMatches(RuntimeDllPath, RuntimeDllBytes, RuntimeDllSha256) || !FileMatches(ProvidersDllPath, ProvidersDllBytes, ProvidersDllSha256))
                {
                    ...
                    Xenostrap.Utility.ResilientDownload.DownloadAsync(App.HttpClient, [RuntimePackageUrl], temp, RuntimePackageBytes, token, RuntimePackageSha256).GetAwaiter().GetResult();
                    try
                    {
                ...
                if (!FileMatches(DirectMlDllPath, DirectMlBytes, DirectMlSha256))
                {
                    ...
                    Xenostrap.Utility.ResilientDownload.DownloadAsync(App.HttpClient, [DirectMlUrl], DirectMlDllPath, DirectMlBytes, token, DirectMlSha256).GetAwaiter().GetResult();
                    if (!FileMatches(DirectMlDllPath, DirectMlBytes, DirectMlSha256))
                    {
        ...
        private static bool EnsureModelFile(ModelSpec spec, CancellationToken token)
        {
            try
            {
                ...
                Xenostrap.Utility.ResilientDownload.DownloadAsync(App.HttpClient, [spec.Url], path, spec.Bytes, token, spec.Sha256).GetAwaiter().GetResult();
