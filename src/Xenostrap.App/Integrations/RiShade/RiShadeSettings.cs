            ...
            try
            {
                lock (_ioLock)
                {
                    if (!File.Exists(FilePath))
                        return;
                    if (Xenostrap.Utility.JsonFile.TryLoad<RiShadeSettings>(FilePath, _jsonOptions, out RiShadeSettings? loaded, out bool recovered, out Exception? failure) && loaded != null)
                    {
                        loaded.Normalize();
                        _current = loaded;
                    }
        ...
        public static void Save()
        {
            try
            {
                lock (_ioLock)
                {
                    ...
                    Xenostrap.Utility.JsonFile.SerializeAtomic(FilePath, _current, _jsonOptions);
                }
            }
            catch (Exception ex)
            {
