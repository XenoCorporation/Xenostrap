...
using Xenostrap.AppData;
using Xenostrap.Core;
using Xenostrap.Enums;
using Xenostrap.Exceptions;
using Xenostrap.Extensions;
using Xenostrap.Integrations;
using Xenostrap.Integrations.AssetProxy;
using Xenostrap.Models;
using Xenostrap.Models.APIs.Roblox;
using Xenostrap.Models.Manifest;
...
using Xenostrap.RobloxInterfaces;
using Xenostrap.UI;
using Xenostrap.UI.Elements.Bootstrapper.Base;
using Xenostrap.UI.Elements.Settings;
...
public class Bootstrapper
{
    ...
    private const string SkyboxArchiveBaseUrl = "https://github.com/nguyenlegiabao13/SkyboxPackV2/archive/";

    private const string SkyboxCommitApiUrl = "https://api.github.com/repos/nguyenlegiabao13/SkyboxPackV2/commits/main";
	...
	private const string VersionOwnershipFileName = ".xenostrap-managed";
    ...
    private static readonly HttpClient SkyboxHttpClient = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromMinutes(30L));

    private static readonly HttpClient RobloxPackageClient = Xenostrap.Utility.VpnHttpClient.Create(Timeout.InfiniteTimeSpan, handler =>
    {
    ...
    private static readonly string? _launchStatusFile = Environment.GetEnvironmentVariable("XENOSTRAP_STATUS_FILE");
    ...
    private void SetStatus(string message)
    {
        ...
        {
            message = message.Replace("{product}", "Xenostrap");
        }
    ...
    public async Task Run()
    {
        ...
        try
        {
            using (Mutex.OpenExisting("Xenostrap-Bootstrapper"))
            {
                App.Logger.WriteLine("Bootstrapper::Run", "Xenostrap-Bootstrapper mutex exists, waiting...");
        ...
        await using AsyncMutex mutex = new(initiallyOwned: false, "Xenostrap-Bootstrapper");
        ...
        Task versionInfoTask = Xenostrap.Utility.Platform.SupportsWindowsClient ? GetLatestVersionInfo(false) : Task.CompletedTask;
        ...
        if (!Xenostrap.Utility.Platform.SupportsWindowsClient)
        {
            Observe(versionInfoTask);
    ...
    private async Task<bool> CheckAndApplyUpdate(string logIdent)
    {
        ...
        if (IsNewerVersion(text2))
        {
            ...
            {
                App.Logger.WriteLine(logIdent, "Update installed restarting Xenostrap...");
	...
	{
		return Xenostrap.AppData.CommonAppData.IsVersionGuidValid(versionGuid);
	}
        ...
        try
        {
            ...
            if (OperatingSystem.IsLinux())
            {
                ...
                OperationResult prepared = await configuration.PrepareAsync(
                    installation,
                    Xenostrap.Utility.SoberConfigurationMapper.CreatePlayerOptions(App.Settings.Prop),
                    cancellationToken);
    ...
    private async Task StartRoblox(CancellationToken ct = default)
    {
        ...
        SetStatus("Starting Roblox");
        await MaybeApplyXenostrapMatchmakerAsync(ct);
        if (!Xenostrap.Utility.Platform.SupportsWindowsClient)
        {
        ...
        try
        {
            ...
            if (App.Settings.Prop.BlockRobloxTelemetry && !TelemetryBlocker.IsApplied())
            {
                ...
                {
                    App.Logger.WriteLine("Bootstrapper::StartRoblox", "Telemetry block entries are missing and Xenostrap is not elevated, skipping reassert");
                }
            }
    ...
    private async Task MaybeApplyXenostrapMatchmakerAsync(CancellationToken ct)
    {
        string rewritten = await RewriteXenostrapMatchmakerBeforeDispatchAsync(
            _launchCommandLine,
            _launchMode,
            ct,
    ...
    internal static async Task<string> RewriteXenostrapMatchmakerBeforeDispatchAsync(
        ...
        Action? onSearching = null)
    {
		...
		if (placeId == 0 || (!App.Settings.Prop.XenostrapMatchmakerEnabled && !ServerMatchmaker.HasPerGamePreference(placeId)) || LaunchInterceptor.ContainsSpecificGameInstance(launchCommandLine))
		{
			return launchCommandLine;
		}
        App.Logger.WriteLine("Bootstrapper::MaybeApplyXenostrapMatchmakerAsync", "Checking the launch for an enabled matchmaker or game datacenter preference, maximum 30 seconds");
        onSearching?.Invoke();
        ...
        switch (result.Outcome)
        {
            case MatchmakerRewriteOutcome.Rewritten:
                App.Logger.WriteLine("Bootstrapper::MaybeApplyXenostrapMatchmakerAsync", "Xenostrap Matchmaker rewrote launch URL to the closest server JobId.");
                break;
            case MatchmakerRewriteOutcome.Unchanged:
                App.Logger.WriteLine("Bootstrapper::MaybeApplyXenostrapMatchmakerAsync", "Xenostrap Matchmaker did not rewrite launch URL.");
                break;
            case MatchmakerRewriteOutcome.Cancelled:
                App.Logger.WriteLine("Bootstrapper::MaybeApplyXenostrapMatchmakerAsync", "Xenostrap Matchmaker timed out, launching with the original URL.");
                break;
            case MatchmakerRewriteOutcome.Failed:
                App.Logger.WriteLine("Bootstrapper::MaybeApplyXenostrapMatchmakerAsync", "Xenostrap Matchmaker error, launching with the original URL: " + result.Error);
                break;
        }
    ...
    private ProcessStartInfo BuildStartInfo()
    {
        ...
        {
            Xenostrap.Utility.EmulationBypassService.ApplyCompatLayerBypass(AppData.ExecutablePath);
            Xenostrap.Utility.EmulationBypassService.ApplyBypassEnvironment(processStartInfo);
        }
        else
        {
            Xenostrap.Utility.EmulationBypassService.RestoreCompatLayers();
            Xenostrap.Utility.EmulationBypassService.RemoveBypassEnvironment(processStartInfo);
        }
    ...
    private async Task LaunchCustomIntegrations(string logIdent, bool preLaunch, CancellationToken ct)
    {
        ...
        foreach (CustomIntegration integration in (enumerable ?? []).Where(i => !i.SpecifyGame && i.PreLaunch == preLaunch))
        {
            ...
            try
            {
                ...
                if (integration.RunMinimized && Xenostrap.Utility.Platform.IsWindows)
                {
                    processStartInfo.WindowStyle = ProcessWindowStyle.Minimized;
                }
                if (integration.RunAsAdmin && Xenostrap.Utility.Platform.IsWindows)
                {
                    processStartInfo.Verb = "runas";
                }
    ...
    private async Task RunStudioSessionAsync(CancellationToken ct)
    {
        ...
        try
        {
            App.Logger.WriteLine("Bootstrapper::RunStudioSessionAsync", "Keeping Xenostrap in the background for the Studio session");
            try
            {
                Xenostrap.Integrations.Studio.StudioIntegration.Start();
            }
            catch
            {
            }
        ...
        finally
        {
            try
            {
                Xenostrap.Integrations.Studio.StudioIntegration.Shutdown();
            }
            catch
            {
            }
    ...
    private Task LaunchWatcherIfNeeded(string logFileName, CancellationToken ct)
    {
        ...
        if (AssetProxyServer.IsRequired && _launchMode == LaunchMode.Player)
        {
            try
            {
                ...
                App.Logger.WriteLine("Bootstrapper::LaunchWatcherIfNeeded", "Running the watcher in-process for AssetWarp so no second Xenostrap process is needed.");
                return Task.CompletedTask;
            }
            catch (Exception ex)
            {
	...
	private bool IsOwnedVersionDirectory(string directory)
	{
		try
		{
			string marker = Xenostrap.Utility.JsonFile.ReadText(Path.Combine(directory, VersionOwnershipFileName), 128).Trim();
    ...
    {
        Xenostrap.Enums.ModApplyTarget target = App.Settings?.Prop?.ModApplyTarget ?? Xenostrap.Enums.ModApplyTarget.Both;
        if (target == Xenostrap.Enums.ModApplyTarget.Both)
            return true;
        return IsStudioLaunch
            ? target == Xenostrap.Enums.ModApplyTarget.Studio
            : target == Xenostrap.Enums.ModApplyTarget.Player;
    }
    ...
    public async Task EnsureSkyboxPackDownloadedAsync()
    {
        ...
        string tempZip = Path.Combine(Path.GetTempPath(), "XenostrapSkybox_" + operationId + ".zip");
    ...
    public static async Task ApplySkyboxPatchToRobloxStorageAsync(CancellationToken ct = default)
    {
        ...
        }, async (item, token) =>
        {
            ...
            try
            {
                using HttpResponseMessage response = await http.GetAsync("https://raw.githubusercontent.com/KloBraticc/SkyboxPatch/main/assets/" + hash, HttpCompletionOption.ResponseHeadersRead, token);
