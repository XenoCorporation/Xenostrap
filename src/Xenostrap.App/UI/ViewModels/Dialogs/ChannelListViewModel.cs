...
namespace Xenostrap.UI.ViewModels.Dialogs
{
    public class ChannelListsViewModel : NotifyPropertyChangedViewModel
    {
        private static readonly string[] ChannelsJsonUrls =
        {
            ...
            "https://raw.githubusercontent.com/XenoCorporation/Xenostrap-Roblox-Channels/main/Channels.json"
        };

        private static readonly HttpClient _channelsClient = Xenostrap.Utility.VpnHttpClient.Create();
        ...
        public async Task RefreshAsync()
        {
            ...
            try
            {
                ...
                foreach (var url in ChannelsJsonUrls)
                {
                    try
                    {
                        json = await Xenostrap.Utility.Http.GetStringBoundedAsync(_channelsClient, url);
        ...
        private async Task SaveCacheToDiskAsync()
        {
            try
            {
				...
				await Task.Run(() => Xenostrap.Utility.JsonFile.SerializeAtomic(CacheFilePath, snapshot)).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
        ...
        private async Task LoadCacheFromDiskAsync()
        {
            try
            {
				...
				var cache = await Task.Run(() => Xenostrap.Utility.JsonFile.Deserialize<Dictionary<string, ClientVersion>>(CacheFilePath, Xenostrap.Utility.JsonOptions.Tolerant, 8388608)).ConfigureAwait(false);
                if (cache != null)
                {
                    lock (_channelInfoCache)
                    {
        ...
        private async Task SaveCacheMetaAsync()
        {
            try
            {
				...
				await Task.Run(() => Xenostrap.Utility.JsonFile.SerializeAtomic(CacheMetaFilePath, snapshot)).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
        ...
        private async Task LoadCacheMetaAsync()
        {
            try
            {
				...
				var meta = await Task.Run(() => Xenostrap.Utility.JsonFile.Deserialize<CacheMeta>(CacheMetaFilePath, Xenostrap.Utility.JsonOptions.Tolerant, 1048576)).ConfigureAwait(false);
                if (meta != null)
                {
