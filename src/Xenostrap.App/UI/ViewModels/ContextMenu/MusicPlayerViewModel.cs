...
using Xenostrap.Integrations.Youtube;
...
public class MusicPlayerViewModel : INotifyPropertyChanged, IDisposable
{
    ...
    private readonly string _youtubeCacheDir = Path.Combine(Paths.Cache, "Youtube", "Downloads");
    ...
    private bool _youtubeHasSearched;
    private double _youtubeProgress;
    private bool _youtubeProgressVisible;
    private CancellationTokenSource? _youtubeSearchCts;
    ...
    public MusicPlayerViewModel()
    {
        ...
        YoutubeSearchCommand = new AsyncRelayCommand(YoutubeSearchAsync, () => !IsYoutubeSearching);
        YoutubePlayCommand = new AsyncRelayCommand<YoutubeSearchResult>(YoutubePlayAsync, _ => !IsYoutubeDownloading);
        YoutubeOpenCommand = new RelayCommand<YoutubeSearchResult>(OpenYoutubeResult);
        CancelYoutubeDownloadCommand = new RelayCommand(CancelYoutubeDownload);
    ...
    public AsyncRelayCommand YoutubeSearchCommand { get; }
    public AsyncRelayCommand<YoutubeSearchResult> YoutubePlayCommand { get; }
    public RelayCommand<YoutubeSearchResult> YoutubeOpenCommand { get; }
    public RelayCommand CancelYoutubeDownloadCommand { get; }
    ...
    public string YoutubeSearchQuery
    {
        get => _youtubeSearchQuery;
        set
        {
            if (_youtubeSearchQuery == value)
                return;
            _youtubeSearchQuery = value ?? string.Empty;
            OnPropertyChanged();
        }
    }
    ...
    public string YoutubeStatus
    {
        ...
        set
        {
            if (_youtubeStatus == value)
                return;
            _youtubeStatus = value;
            OnPropertyChanged();
        }
    }
    ...
    {
        get => _isYoutubeSearching;
        private set
        {
            if (_isYoutubeSearching == value)
                return;
            _isYoutubeSearching = value;
            OnPropertyChanged();
            OnPropertyChanged(nameof(YoutubeHasNoResults));
            YoutubeSearchCommand.NotifyCanExecuteChanged();
        }
    }
    ...
    public bool IsYoutubeDownloading
    {
        ...
        private set
        {
            if (_isYoutubeDownloading == value)
                return;
            _isYoutubeDownloading = value;
            OnPropertyChanged();
            YoutubePlayCommand.NotifyCanExecuteChanged();
        }
    }

    public bool YoutubeProgressVisible
    {
        get => _youtubeProgressVisible;
        private set
        {
            if (_youtubeProgressVisible == value)
                return;
            _youtubeProgressVisible = value;
            OnPropertyChanged();
        }
    }

    public double YoutubeProgress
    {
        get => _youtubeProgress;
        private set
        {
            if (Math.Abs(_youtubeProgress - value) < 0.1)
                return;
            _youtubeProgress = value;
            OnPropertyChanged();
        }
    }

    public bool HasYoutubeResults => YoutubeResults.Count > 0;

    public bool YoutubeHasNoResults => _youtubeHasSearched && !IsYoutubeSearching && YoutubeResults.Count == 0;
    ...
    private void DeleteTrackFile(TrackItem track)
    {
        if (string.IsNullOrEmpty(track.FilePath) || !string.Equals(track.FileType, "YOUTUBE", StringComparison.OrdinalIgnoreCase))
            return;
        string cacheRoot;
        try
        {
            cacheRoot = Path.GetFullPath(_youtubeCacheDir).TrimEnd('\\') + "\\";
        }
        catch
        {
            return;
        ...
        try
        {
            file = Path.GetFullPath(track.FilePath);
        }
        catch
        {
            return;
        }
        if (!file.StartsWith(cacheRoot, StringComparison.OrdinalIgnoreCase) || !File.Exists(file))
            return;
        try
        {
            File.Delete(file);
            Status = "Removed and deleted the downloaded file: " + track.Title;
        }
        catch (Exception ex)
        {
            App.Logger.WriteLine("MusicPlayer", "Could not delete downloaded file " + file + ": " + ex.Message);
        }
    }
    ...
    private void UpdateRpcPresence(bool force = false)
    {
        ...
        if (NowPlaying == null || string.IsNullOrWhiteSpace(NowPlaying.Title))
        {
            ...
            {
                Details = "Idle",
                State = "Xenostrap Music Player",
                Assets = new Assets
                {
                    LargeImageKey = App.WebsiteBaseUrl + "/Image/Xenostrap.png",
                    LargeImageText = "Xenostrap Music Player"
                }
            };
        }
        else
    ...
    private void SaveLibrary()
    {
        try
        {
			...
			Task.Run(() =>
            {
				try { Xenostrap.Utility.JsonFile.SerializeAtomic(path, data, Xenostrap.Utility.JsonOptions.Indented); }
                catch { }
            });
        }
        catch (Exception ex)
    ...
    private void LoadLibrary()
    {
        try
        {
            if (!File.Exists(_savePath))
                return;
			JsonElement root = Xenostrap.Utility.JsonFile.Deserialize<JsonElement>(_savePath, Xenostrap.Utility.JsonOptions.Tolerant, 16777216);
    ...
    private void LoadEqualizer()
    {
        try
        {
            if (!File.Exists(_eqPath))
                return;
			JsonElement root = Xenostrap.Utility.JsonFile.Deserialize<JsonElement>(_eqPath, Xenostrap.Utility.JsonOptions.Tolerant, 4194304);
    ...
    private async Task YoutubeSearchAsync()
    {
        string query = YoutubeSearchQuery?.Trim() ?? string.Empty;
        if (query.Length == 0)
        {
            YoutubeStatus = "Enter a search query first.";
            return;
        }

        _youtubeSearchCts?.Cancel();
        var cts = new CancellationTokenSource();
        _youtubeSearchCts = cts;
        try
        {
            IsYoutubeSearching = true;
            YoutubeResults.Clear();
            OnPropertyChanged(nameof(HasYoutubeResults));
            OnPropertyChanged(nameof(YoutubeHasNoResults));
            YoutubeStatus = "Searching YouTube for \"" + query + "\"...";

            IReadOnlyList<YoutubeSearchResult> results = await Task.Run(
                () => YoutubeSearchService.SearchAsync(query, 40, cts.Token), cts.Token).ConfigureAwait(true);

            if (cts.Token.IsCancellationRequested)
                return;

            _youtubeHasSearched = true;
            YoutubeResults.Clear();
            foreach (YoutubeSearchResult result in results)
                YoutubeResults.Add(result);
            YoutubeStatus = results.Count > 0
                ? "Found " + results.Count + " results for \"" + query + "\". Click a song to download and play."
        ...
        {
            YoutubeStatus = "Search failed: " + ex.Message;
        }
        finally
        {
            ...
            OnPropertyChanged(nameof(HasYoutubeResults));
            OnPropertyChanged(nameof(YoutubeHasNoResults));
            if (ReferenceEquals(_youtubeSearchCts, cts))
            {
                _youtubeSearchCts = null;
                cts.Dispose();
            }
        }
    }

    private async Task YoutubePlayAsync(YoutubeSearchResult? result)
    {
        if (result == null || string.IsNullOrEmpty(result.VideoId) || IsYoutubeDownloading)
            return;

        _youtubeDownloadCts?.Cancel();
        var cts = new CancellationTokenSource();
        _youtubeDownloadCts = cts;

        string baseName = "yt_" + result.VideoId;
        string expectedFile = Path.Combine(_youtubeCacheDir, baseName + ".m4a");

        try
        {
            IsYoutubeDownloading = true;
            YoutubeProgressVisible = true;
            YoutubeProgress = 0;
            YoutubeStatus = "Preparing to download: " + result.Title;

            string filePath = expectedFile;
            if (!File.Exists(expectedFile))
            {
                filePath = await Task.Run(
                    () => YoutubeToolchain.DownloadAudioAsync(result.Url, _youtubeCacheDir, baseName, OnYoutubeProgress, cts.Token), cts.Token).ConfigureAwait(true);
            }

            if (cts.Token.IsCancellationRequested)
                return;

            TrackItem? existing = Tracks.FirstOrDefault(t => string.Equals(t.FilePath, filePath, StringComparison.OrdinalIgnoreCase));
            TrackItem item;
            if (existing != null)
            {
                item = existing;
            ...
            else
            {
                item = new TrackItem
                {
                    ...
                    FileType = "YOUTUBE",
                    Duration = TimeSpan.FromSeconds(Math.Max(0, result.DurationSeconds))
                };
                Tracks.Add(item);
                ProbeDurationAsync(item);
            }
            ...
            SelectedTrack = item;
            YoutubeStatus = "Playing: " + result.Title;
        }
        catch (OperationCanceledException) when (cts.Token.IsCancellationRequested)
        {
            YoutubeStatus = "Download cancelled.";
        }
        catch (Exception ex)
        {
        ...
        finally
        {
            ...
            YoutubeProgressVisible = false;
            if (ReferenceEquals(_youtubeDownloadCts, cts))
            {
    ...
    private void OnYoutubeProgress(string label, double fraction, bool _)
    {
        Application.Current?.Dispatcher.BeginInvoke(() =>
        {
            if (fraction >= 0.0)
                YoutubeProgress = Math.Clamp(fraction * 100.0, 0.0, 100.0);
            if (!string.IsNullOrEmpty(label))
                YoutubeStatus = label;
        });
    }

    private void CancelYoutubeDownload()
    {
        try
        {
            _youtubeDownloadCts?.Cancel();
    ...
    private void OpenYoutubeResult(YoutubeSearchResult? result)
    {
        ...
        {
            YoutubeStatus = "Failed to open YouTube: " + ex.Message;
        }
    }
    ...
    public void Dispose()
    {
        ...
        CancellationTokenSource? youtubeSearchCts = _youtubeSearchCts;
        _youtubeSearchCts = null;
        if (youtubeSearchCts != null)
        {
            try
            {
                youtubeSearchCts.Cancel();
            }
            catch (ObjectDisposedException)
            {
            }
            youtubeSearchCts.Dispose();
        }
        CancellationTokenSource? youtubeDownloadCts = _youtubeDownloadCts;
        _youtubeDownloadCts = null;
        if (youtubeDownloadCts != null)
        {
            try
            {
                youtubeDownloadCts.Cancel();
            }
            catch (ObjectDisposedException)
            {
            }
            youtubeDownloadCts.Dispose();
        }
        if (_rpcConnected && _rpcClient != null)
        {
            try
