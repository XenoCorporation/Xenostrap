...
internal class ServerInformationViewModel : NotifyPropertyChangedViewModel, IDisposable
{
	...
	private async Task FetchUsernameAsync()
	{
		try
		{
			...
			using JsonDocument jsonDocument = JsonDocument.Parse(await Xenostrap.Utility.Http.GetString($"https://users.roblox.com/v1/users/{num}"));
	...
	private async Task SetGameIcon(string? url)
	{
		...
		try
		{
			BitmapSource? icon = await Task.Run(() => Xenostrap.Utility.AppImage.LoadSync(url));
			if (icon != null)
			{
				GameIcon = icon;
			}
