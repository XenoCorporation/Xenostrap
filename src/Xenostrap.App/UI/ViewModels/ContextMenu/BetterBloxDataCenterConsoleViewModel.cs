...
{
	private static readonly HttpClient _http = Xenostrap.Utility.VpnHttpClient.Create();
	...
	private async Task LoadDatacentersAsync()
	{
		try
		{
			using JsonDocument jsonDocument = JsonDocument.Parse(await Xenostrap.Utility.Http.GetStringBoundedAsync(_http, "https://api.betterroblox.com/servers/datacenters"));
