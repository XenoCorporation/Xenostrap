...
public class RPCCustomizerViewModel : INotifyPropertyChanged, IDisposable
{
	...
	public RPCCustomizerViewModel()
	{
		...
		_button2Label = "Join";
		_button2Url = "https://discord.gg/Ay7GhEvxYZ";
		_statusMessage = "Idle";
	...
	private void UpdatePresence()
	{
		...
		try
		{
			...
			{
				Details = (string.IsNullOrWhiteSpace(Details) ? "Using Xenostrap" : Details),
				State = State,
				Assets = new Assets
				{
	...
	private void SaveConfigInternal()
	{
		try
		{
			Xenostrap.Utility.JsonFile.SerializeAtomic(_configPath, new RpcConfig(_applicationId, _appName, _details, _state, _largeImageKey, _smallImageKey, _button1Enabled, _button2Enabled, _button1Label, _button1Url, _button2Label, _button2Url, _autoStartRpc), Xenostrap.Utility.JsonOptions.Indented);
	...
	private void LoadConfigInternal()
	{
		...
		try
		{
			...
			RpcConfig rpcConfig = Xenostrap.Utility.JsonFile.Deserialize<RpcConfig>(_configPath, Xenostrap.Utility.JsonOptions.Tolerant, 4194304);
			if (!(rpcConfig == null))
			{
