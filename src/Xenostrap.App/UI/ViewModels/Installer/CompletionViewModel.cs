...
public class CompletionViewModel : ObservableObject, IDisposable
{
	...
	private static bool RestartInto(string flags)
	{
		...
		try
		{
			...
			App.Logger.WriteLine("CompletionViewModel", "Restarting Xenostrap with " + flags);
			App.Terminate();
			return true;
		}
		catch (Exception ex)
		{
			App.Logger.WriteLine("CompletionViewModel", "Could not restart Xenostrap: " + ex.Message);
			return false;
		}
	}
