...
{
	private static readonly HttpClient _httpClient = Xenostrap.Utility.VpnHttpClient.Create();
	...
	public ShortcutTask DesktopIconTask { get; } = new ShortcutTask("Desktop", Paths.Desktop, "Xenostrap.lnk");

	public ShortcutTask StartMenuIconTask { get; } = new ShortcutTask("StartMenu", Paths.WindowsStartMenu, "Xenostrap.lnk");
	...
	private async Task LoadGameNameAsync(string gameId)
	{
		...
		try
		{
			...
			using JsonDocument uniDoc = JsonDocument.Parse(await Xenostrap.Utility.Http.GetStringBoundedAsync(_httpClient, requestUri));
			...
			using JsonDocument jsonDocument = JsonDocument.Parse(await Xenostrap.Utility.Http.GetStringBoundedAsync(_httpClient, requestUri2));
	...
	private async Task<string> FetchGameIconAsync(string gameId)
	{
		...
		using JsonDocument jsonDocument = JsonDocument.Parse(await Xenostrap.Utility.Http.GetStringBoundedAsync(_httpClient, requestUri).ConfigureAwait(continueOnCapturedContext: false));
		JsonElement property = jsonDocument.RootElement.GetProperty("data");
