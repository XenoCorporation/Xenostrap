...
public class IntegrationsViewModel : NotifyPropertyChangedViewModel, IDisposable
{
	...
	public bool UncapFpsToggleEnabled => Xenostrap.Integrations.FrameGeneration.FrameGenSettings.ModeIndex == 0;
	...
	private async Task ApplyTelemetrySettingAsync(bool value, int version)
	{
		try
		{
			...
			await Application.Current.Dispatcher.InvokeAsync(() =>
			{
				...
				_blockTelemetry = !value;
				OnPropertyChanged("BlockTelemetry");
				Frontend.ShowMessageBox(value ? "Xenostrap could not enable the telemetry blocker. Administrator approval is required to edit the hosts file." : "Xenostrap could not disable the telemetry blocker. Administrator approval is required to edit the hosts file.", MessageBoxImage.Warning);
			});
		}
