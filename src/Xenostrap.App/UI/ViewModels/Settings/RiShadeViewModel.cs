...
{
    public class RiShadeViewModel : Xenostrap.UI.ViewModels.NotifyPropertyChangedViewModel
    {
        ...
        private void SavePreset()
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(name))
                    return;
				Xenostrap.Utility.JsonFile.SerializeAtomic(Path.Combine(PresetsDir, name + ".json"), RiShadeSettings.Current, Xenostrap.Utility.JsonOptions.Indented);
        ...
        private void LoadCustomPreset(string name)
        {
            try
            {
                ...
                if (!File.Exists(path))
                    return;
				var loaded = Xenostrap.Utility.JsonFile.Deserialize<RiShadeSettings>(path, Xenostrap.Utility.JsonOptions.Tolerant, 4194304);
        ...
        private void ImportPreset()
        {
            try
            {
                ...
                if (dlg.ShowDialog() != true)
                    return;
				var loaded = Xenostrap.Utility.JsonFile.Deserialize<RiShadeSettings>(dlg.FileName, Xenostrap.Utility.JsonOptions.Tolerant, 4194304);
        ...
        private void ExportPreset()
        {
            try
            {
                ...
                if (dlg.ShowDialog() != true)
                    return;
				Xenostrap.Utility.JsonFile.SerializeAtomic(dlg.FileName, RiShadeSettings.Current, Xenostrap.Utility.JsonOptions.Indented, false);
            }
            catch (Exception ex)
            {
