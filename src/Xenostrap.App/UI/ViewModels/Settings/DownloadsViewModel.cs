...
using CoreBootstrapper = Xenostrap.Bootstrapper;
    ...
    public class DownloadsViewModel : NotifyPropertyChangedViewModel
    {
        ...
        public class DownloadItem : NotifyPropertyChangedViewModel
        {
            ...
            private async Task ChangeLocationAsync()
            {
                ...
                if (Frontend.ShowMessageBox($"{Title} will be installed at:\n{target}\n\nOnly {Title} moves, Xenostrap and the other Roblox app stay where they are. Continue?", MessageBoxImage.Question, MessageBoxButton.YesNo, MessageBoxResult.No) != MessageBoxResult.Yes)
                {
                    _parent.EndOperation();
                    return;
                }
            ...
            private static ImageSource LoadIcon(string uri)
            {
                try
                {
                    return Xenostrap.Utility.AppImage.LoadSync(uri);
                }
                catch
                {
                    return null;
        ...
        public class ClientItem : NotifyPropertyChangedViewModel
        {
            ...
            private async Task InstallOrReinstallAsync()
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Frontend.ShowMessageBox($"Could not install {Title}: {ex.Message}\n\nCheck the classic download URL in Settings, or set a local XenostrapClient folder to install from.", MessageBoxImage.Error);
                }
                finally
                {
                    _cts?.Dispose();
        ...
        public void RefreshClassic()
        {
            ...
            if (!ClassicClients.EngineInstalled)
                ClassicStatusText = "Install any version to set up the classic engine. Clients download from the classic download URL (set in Settings), or from a local XenostrapClient folder if you set one.";
            else
        ...
        private void LocateClassicSource()
        {
            var dialog = new OpenFolderDialog
            {
                Title = "Locate your XenostrapClient folder",
                Multiselect = false
            };
            if (dialog.ShowDialog() != true || string.IsNullOrWhiteSpace(dialog.FolderName))
                return;
            ...
            {
                Frontend.ShowMessageBox("That folder does not look like an XenostrapClient installation. It must contain XenostrapClient.WebServer.exe and a data\\clients folder.", MessageBoxImage.Warning);
                return;
            }
