...
namespace Xenostrap.UI.ViewModels.Settings
{
    ...
    public sealed partial class NewsViewModel : ObservableObject, IDisposable
    {
        ...
        public const string FeedUrl =
            "https://raw.githubusercontent.com/nguyenlegiabao13/XenostrapNews-/refs/heads/main/news.json";
        ...
        {
            var http = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromSeconds(15), handler =>
                handler.AutomaticDecompression = DecompressionMethods.GZip | DecompressionMethods.Deflate);
            http.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("Xenostrap", "1.0"));
            return http;
        }
            ...
            try
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        json = await Xenostrap.Utility.Http.ReadStringBoundedAsync(resp.Content, MaxNewsJsonBytes, ct);
                ...
                var imageTasks = items.Select(async item =>
                {
                    ...
                    try
                    {
                        ...
                        if (!File.Exists(localPath) || !fromCache)
                        {
                            ...
                            var data = await Xenostrap.Utility.Http.ReadBytesBoundedAsync(imageResponse.Content, MaxNewsImageBytes, ct);
        ...
        private static async Task<BitmapSource?> LoadLocalImageAsync(string path, CancellationToken ct)
        {
            try
            {
                ...
                {
                    return Xenostrap.Utility.SafeImaging.FromBytes(data, 900);
                }, ct);
            }
            catch (Exception ex)
            {
