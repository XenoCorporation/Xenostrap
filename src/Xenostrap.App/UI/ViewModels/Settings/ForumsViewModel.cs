...
namespace Xenostrap.UI.ViewModels.Settings
{
    ...
    public sealed partial class ForumsViewModel : ObservableObject
    {
        ...
        private static ImageSource? DecodeImageBytes(byte[] bytes, int decodePixelWidth)
        {
            try
            {
                var image = Xenostrap.Utility.SafeImaging.FromBytes(bytes, decodePixelWidth);
                if (image != null)
                {
                    return image;
                }
        ...
        private static async Task<ImageSource?> LoadAvatarAsync(string avatar)
        {
            ...
            else
            {
                ...
                if (Uri.TryCreate(url, UriKind.Absolute, out Uri? uri) && IsAllowedAvatarHost(uri))
                {
                    try
                    {
                        result = await Xenostrap.Utility.AppImage.LoadAsync(url, 96).ConfigureAwait(false);
                    }
                    catch
                    {
                    }
        ...
        {
            return Xenostrap.Utility.AppImage.LoadAsync(url, 38, CancellationToken.None);
        }
