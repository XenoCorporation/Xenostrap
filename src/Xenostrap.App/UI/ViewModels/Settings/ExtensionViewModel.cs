...
namespace Xenostrap.UI.Elements.Settings.Pages
{
    public class ExtensionViewModel : NotifyPropertyChangedViewModel
    {
        ...
        {
            var http = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromMinutes(10));
        ...
        public ExtensionViewModel()
        {
            ...
            if (App.Settings.Prop.RojoEnabled && !Xenostrap.Integrations.Rojo.RojoManager.IsInstalled)
                _ = DownloadRojoAsync();
        }
        ...
        private bool StudioPluginVisible => ShowType(TypeStudio) && MatchesSearch("Xenostrap Studio plugin panel Discord rich presence rpc place script mode selection Roblox Studio Extensions");
        ...
        private string _riShadeOpenLabel = Xenostrap.Integrations.RiShade.RiShadePanel.IsOpen ? "Close" : "Open";
        ...
        public bool studiopluginenabler
        {
            ...
            set
            {
                ...
                if (value)
                {
                    Xenostrap.Integrations.Studio.StudioPluginInstaller.EnsureInstalled(force: true);
                    return;
                }
                Xenostrap.Integrations.Studio.StudioIntegration.Shutdown();
                Xenostrap.Integrations.Studio.StudioPluginInstaller.Uninstall();
            }
        }
        ...
        public string RojoServeLabel => Xenostrap.Integrations.Rojo.RojoManager.IsServing ? "Stop sync" : "Start sync";
        ...
        private async Task DownloadRojoAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    await Xenostrap.Integrations.Rojo.RojoManager.EnsureInstalledAsync(
        ...
        private async Task UninstallRojoAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    await Xenostrap.Integrations.Rojo.RojoManager.UninstallAsync();
                }
                finally
                {
                    _gate.Release();
        ...
        {
            if (!Xenostrap.Integrations.Rojo.RojoManager.IsInstalled)
            {
        ...
        public async Task RojoInitAsync()
        {
            ...
            try
            {
                ...
                var (ok, output) = await Xenostrap.Integrations.Rojo.RojoManager.RunAsync("init", RojoProjectPath, ct);
                OnProgressChanged?.Invoke("", -1.0, false);
                if (ok)
        ...
        {
            if (Xenostrap.Integrations.Rojo.RojoManager.IsServing)
            {
                Xenostrap.Integrations.Rojo.RojoManager.StopServe();
                OnPropertyChanged(nameof(RojoServeLabel));
                return;
            }
            if (!EnsureRojoReady())
                return;
            bool started = Xenostrap.Integrations.Rojo.RojoManager.StartServe(RojoProjectPath);
            if (!started)
        ...
        public async Task RojoBuildAsync()
        {
            ...
            try
            {
                ...
                var (ok, output) = await Xenostrap.Integrations.Rojo.RojoManager.RunAsync($"build -o \"{outFile}\"", RojoProjectPath, ct);
                OnProgressChanged?.Invoke("", -1.0, false);
                if (ok)
        ...
        public async Task RojoInstallPluginAsync()
        {
            ...
            try
            {
                ...
                var (ok, output) = await Xenostrap.Integrations.Rojo.RojoManager.RunAsync("plugin install", RojoProjectPath, ct);
                OnProgressChanged?.Invoke("", -1.0, false);
                if (ok)
        ...
        {
            await Xenostrap.Utility.ResilientDownload.DownloadAsync(client, urls, outputPath, MaxDownloadBytes, ct, expectedDigest,
                (read, total) =>
                {
        ...
        private static async Task<(IReadOnlyList<string> Urls, string Digest)> ResolveFleasionUrlsAsync(CancellationToken ct)
        {
            ...
            try
            {
                using JsonDocument doc = JsonDocument.Parse(await Xenostrap.Utility.Http.GetStringBoundedAsync(client, "https://api.github.com/repos/fleasion/Fleasion/releases/latest", ct));
        ...
        private static async Task<(IReadOnlyList<string> Urls, string Digest)> ResolveApiDumpUrlsAsync(CancellationToken ct)
        {
            ...
            try
            {
                using JsonDocument doc = JsonDocument.Parse(await Xenostrap.Utility.Http.GetStringBoundedAsync(client, "https://api.github.com/repos/MaximumADHD/Roblox-API-Dump-Tool/releases/latest", ct));
                foreach (JsonElement asset in doc.RootElement.GetProperty("assets").EnumerateArray())
                {
                    string name = asset.GetProperty("name").GetString() ?? "";
        ...
        {
            var (cts, ct) = BeginOperation();
            string tempZip = Path.Combine(Path.GetTempPath(), "xenostrap-community-content.zip");
            string tempDir = Path.Combine(Path.GetTempPath(), "xenostrap-community-content");
            try
            {
                await _gate.WaitAsync(ct);
                try
                {
                    ...
                    Xenostrap.Utility.SafeZipExtractor.ExtractToDirectory(tempZip, tempDir, maxExpandedBytes: 2147483648L);
