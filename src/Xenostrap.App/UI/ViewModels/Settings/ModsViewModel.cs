...
using Xenostrap.Models.SettingTasks;
...
public class ModsViewModel : NotifyPropertyChangedViewModel
{
	...
	private static readonly string RepoRoot = "https://api.github.com/repos/nguyenlegiabao13/SkyboxPackV2/contents";
	...
	{
		HttpClient client = Xenostrap.Utility.VpnHttpClient.Create();
		client.DefaultRequestHeaders.UserAgent.ParseAdd("XenostrapApp");
		return client;
	}
	...
	public bool IsWindows => Xenostrap.Utility.Platform.IsWindows;
...
public ICommand PickCursorColorCommand { get; }
	...
	public IEnumerable<Xenostrap.Enums.ModApplyTarget> ModApplyTargets { get; } = Enum.GetValues<Xenostrap.Enums.ModApplyTarget>();

	public Xenostrap.Enums.ModApplyTarget ModApplyTarget
	{
	...
	public EnumModPresetTask<Xenostrap.Enums.CursorType> CursorTypeTask { get; } = new EnumModPresetTask<Xenostrap.Enums.CursorType>("CursorType", new Dictionary<Xenostrap.Enums.CursorType, Dictionary<string, string>>
	{
		{
	...
	public bool HomepageBackgroundOverlayEnabled
	{
		...
		set
		{
			...
			OnPropertyChanged();
			Xenostrap.Integrations.Overlays.OverlayHub.Refresh();
			if (value)
	...
	{
		get => Xenostrap.Integrations.Overlays.OverlaySettings.HomepageBackgroundMode switch
		{
			"Gradient" => "Gradient",
		...
		set
		{
			...
			Xenostrap.Integrations.Overlays.OverlayHub.Restart();
		}
	}
	...
	public bool RiShadeEnabled
	{
		...
		set
		{
			Xenostrap.Integrations.RiShade.RiShadeManager.SetEnabled(value);
			OnPropertyChanged(nameof(RiShadeEnabled));
		}
	}

	public string[] AntiAliasingMethodNames => Xenostrap.Integrations.AntiAliasing.AntiAliasingSettings.MethodNames;

	public int AntiAliasingMethodIndex
	{
		get
		{
			return Xenostrap.Integrations.AntiAliasing.AntiAliasingSettings.MethodIndex;
		}
		set
		{
			if (value < 0)
				return;
			Xenostrap.Integrations.AntiAliasing.AntiAliasingManager.SetMethod(value);
	...
	public bool FrameGenEnabled
	{
		get
		{
			return Xenostrap.Integrations.FrameGeneration.FrameGenSettings.ModeIndex > 0;
		}
		set
		{
			Xenostrap.Integrations.FrameGeneration.FrameGenManager.SetMode(value ? 1 : 0);
	...
	private const int FrameGenBestBelowFps = Xenostrap.Integrations.Overlays.RobloxFpsCap.BestBelowFps;
	...
	{
		Xenostrap.Integrations.Overlays.RobloxFpsCap.EnsureStarted();
		bool fgOn = Xenostrap.Integrations.FrameGeneration.FrameGenSettings.ModeIndex > 0;
		int hz = Xenostrap.Integrations.Overlays.OverlayDisplay.RefreshHz();
		int cap = Xenostrap.Integrations.Overlays.RobloxFpsCap.Cap;
		double measured = Xenostrap.Integrations.Overlays.RobloxFpsCap.RecentMeasuredBase();
		int suggest = Xenostrap.Integrations.Overlays.RobloxFpsCap.PickBestCap(hz, measured);
		bool capTooHigh = Xenostrap.Integrations.Overlays.RobloxFpsCap.IsUnlimited || cap > hz;
		...
		if (capTooHigh)
			FrameGenCapWarningMessage = $"Your Roblox FPS cap is {Xenostrap.Integrations.Overlays.RobloxFpsCap.Describe()}, at or above your {hz}Hz display. Cap Roblox at {suggest} instead.";
	...
	private void FixFrameCap()
	{
		int hz = Xenostrap.Integrations.Overlays.OverlayDisplay.RefreshHz();
		double measured = Xenostrap.Integrations.Overlays.RobloxFpsCap.RecentMeasuredBase();
		int previousCap = Xenostrap.Integrations.Overlays.RobloxFpsCap.Cap;
		int cap = Xenostrap.Integrations.Overlays.RobloxFpsCap.PickBestCap(hz, measured);
		Xenostrap.Integrations.FrameGeneration.FrameGenManager.SetTargetCap(cap);
		RefreshFrameGenWarning();
	...
	public System.Collections.Generic.IReadOnlyList<Xenostrap.Utility.ClockDisplay.ZoneOption> ClockTimeZones => Xenostrap.Utility.ClockDisplay.Options;
	...
	public string ClockTimeZoneId
	{
		...
		set
		{
			...
			Xenostrap.Utility.ClockDisplay.Invalidate();
		}
	}
	...
	public string AssetWarpStatus
{
	...
	public async Task LoadSkyboxPacksFromGithub(bool force = false)
	{
		...
		try
		{
			...
			byte[] data = await Xenostrap.Utility.Http.ReadBytesBoundedAsync(response.Content, 2097152, cancellation.Token).ConfigureAwait(false);
	...
	private void PickHomepageBackgroundColor()
	{
		...
		var dialog = new Xenostrap.UI.Elements.Controls.RinColorPickerDialog(initial);
	...
	{
		Xenostrap.UI.Elements.Dialogs.TextInputDialog dialog = new(title, initial);
	...
	{
		var dlg = new Xenostrap.UI.Elements.Controls.RinColorPickerDialog();
	...
	private ImageSource LoadImageFromUrl(string url)
	{
		try
		{
			return Xenostrap.Utility.AppImage.LoadSync(url);
		}
		catch
		{
			return null;
	...
	{
		if (!Xenostrap.Utility.SafeImaging.SupportsVisualCapture)
		{
			Frontend.ShowMessageBox(Strings.Common_NotAvailableOnPlatform, MessageBoxImage.Information);
			return;
		}
	...
	private static BitmapSource LoadImageSafely(string path)
	{
		...
		try
		{
			return Xenostrap.Utility.SafeImaging.FromFile(path);
		}
		catch
		{
			return null;
	...
	private string GetRobloxPlayerDir()
	{
		...
		if (Xenostrap.AppData.CommonAppData.IsVersionGuidValid(playerData.State.VersionGuid))
		{
		...
		if (Directory.Exists(versionsRoot))
		{
			try
			{
				...
				{
					if (Xenostrap.AppData.CommonAppData.IsVersionGuidValid(Path.GetFileName(text2)) && File.Exists(Path.Combine(text2, "RobloxPlayerBeta.exe")))
					{
						return text2;
					}
				}
	...
	private async Task RefreshModFilesAsync(CancellationToken token)
	{
		...
		catch (Exception ex)
		{
			...
			status = Directory.Exists(path)
				...
				: "The Roblox install folder was not found. Launch Roblox once through Xenostrap, then refresh.";
		}
