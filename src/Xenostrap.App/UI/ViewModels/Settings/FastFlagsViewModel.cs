...
using Xenostrap.Enums.FlagPresets;
...
public class FastFlagsViewModel : NotifyPropertyChangedViewModel
{
	...
	public bool HasNvidiaGpu => Xenostrap.Utility.GpuInventory.HasNvidia;

	public string NvidiaTabUnavailableReason => Xenostrap.Utility.GpuInventory.HasNvidia
		? string.Empty
		: "These options need an NVIDIA graphics card. Detected: " + Xenostrap.Utility.GpuInventory.Summary + ".";
	...
	public bool AssetWarpEnabled
	{
		...
		set
		{
			...
			if (!value && Xenostrap.Utility.Platform.IsLinux)
			{
	...
	private static async Task RemoveLinuxAssetWarpCertificateAsync()
	{
		...
		try
		{
			...
			Xenostrap.Platform.OperationResult result = await LinuxAssetWarpBridge.RemoveCertificateAsync().ConfigureAwait(false);
			if (result.Succeeded)
			{
