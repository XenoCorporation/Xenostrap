...
public partial class InlineVideoPlayer : UserControl, IDisposable
{
    ...
    private async Task StartAsync()
    {
        ...
        try
        {
            CoreWebView2EnvironmentOptions options = new CoreWebView2EnvironmentOptions(Xenostrap.Utility.RenderAcceleration.ApplyToBrowserArguments(BrowserArguments));
    ...
    private static string GetUserDataFolder()
    {
        ...
        if (string.IsNullOrWhiteSpace(folder))
            folder = Path.Combine(Path.GetTempPath(), "Xenostrap", "WebView2");
        try
        {
