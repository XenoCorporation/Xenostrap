...
namespace Xenostrap.UI.Elements.Controls
{
    public sealed class ForumRichContent : StackPanel
    {
        ...
        private void OpenLightbox(ImageSource inlineSource, byte[]? fullBytes, string meta)
        {
            try
            {
                ...
                if (fullBytes != null)
                {
					ImageSource? full = Xenostrap.Utility.SafeImaging.FromBytes(fullBytes, 2560);
					if (full != null)
						display = full;
                }
                ...
                var window = new Window
                {
					...
					AllowsTransparency = Xenostrap.Utility.Platform.IsWindows,
					Background = Xenostrap.Utility.Platform.IsWindows ? Brushes.Transparent : Brushes.Black,
