...
namespace Xenostrap.UI.Elements.Base;
...
public abstract class WpfUiWindow : UiWindow, IDisposable
{
	...
	private static readonly Dictionary<Xenostrap.Enums.Theme, ResourceDictionary> _builtInThemeCache = new Dictionary<Xenostrap.Enums.Theme, ResourceDictionary>();

	private Xenostrap.Enums.Theme? _lastAppliedTheme;
	...
	{
		Xenostrap.UI.RoundedWindowChrome.Prepare(this);
		ApplyTheme();
	}

	public void ApplyTheme()
	{
		Xenostrap.Enums.Theme final = App.Settings.Prop.Theme2.GetFinal();
		bool flag = final == Xenostrap.Enums.Theme.Custom;
		if (flag || _lastAppliedTheme != final)
		{
			ThemeType theme = ((final != Xenostrap.Enums.Theme.Light) ? ThemeType.Dark : ThemeType.Light);
			try
			{
				...
				{
					Wpf.Ui.Appearance.Accent.Apply(Xenostrap.Utility.SystemAccent.Get(), theme);
				}
			}
			catch (Exception ex)
			{
	...
	private ResourceDictionary? LoadCustomThemeDict()
	{
		try
		{
			return Xenostrap.Utility.CustomTheme.LoadForApp();
		}
		catch (Exception ex)
		{
	...
	private static ResourceDictionary? LoadBuiltInThemeDict(Xenostrap.Enums.Theme theme)
	{
		if (_builtInThemeCache.TryGetValue(theme, out ResourceDictionary? cached))
		{
			return cached;
		}
		string text = Enum.GetName(typeof(Xenostrap.Enums.Theme), theme) ?? "Dark";
	...
	private static readonly bool IsWindows11OrNewer = Xenostrap.Utility.Platform.IsWindows && OperatingSystem.IsWindowsVersionAtLeast(10, 0, 22000);
	...
	protected override void OnSourceInitialized(EventArgs e)
	{
		...
		if (Icon == null)
		{
			try
			{
				Icon = System.Windows.Media.Imaging.BitmapFrame.Create(new Uri("pack://application:,,,/Xenostrap.png", UriKind.Absolute));
			}
			catch (Exception ex)
			{
		...
		ApplyRoundedCorners();
		Xenostrap.UI.WindowBackdrop.Apply(this);
	}

	protected override void OnActivated(EventArgs e)
	{
		base.OnActivated(e);
		Xenostrap.Utility.MemoryManager.SetActive();
	}

	protected override void OnDeactivated(EventArgs e)
	{
		base.OnDeactivated(e);
		Xenostrap.Utility.MemoryManager.SetBackground();
	}
	...
	private void ApplyWin10RoundRegion()
	{
		...
		if (Xenostrap.UI.WindowBackdrop.HasBackdrop(this))
		{
			hwndSource.CompositionTarget.BackgroundColor = System.Windows.Media.Colors.Transparent;
		}
		else
		{
			System.Windows.Media.Color surface = Xenostrap.UI.WindowBackdrop.CreateSurfaceColor();
	...
	{
		if (msg == WmGetMinMaxInfo && Xenostrap.Utility.Platform.IsWindows)
		{
