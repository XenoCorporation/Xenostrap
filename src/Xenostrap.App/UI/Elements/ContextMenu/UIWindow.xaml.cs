...
namespace Xenostrap.UI.Elements.Overlay
{
    public class OverlayWindow : Window, INotifyPropertyChanged
    {
        ...
        private void OnUIWindowClosed(object? sender, EventArgs e)
        {
            ...
            try
            {
                Xenostrap.Utility.ScreenColorEffect.Reset();
            }
            catch
            {
            }
        ...
        {
            Xenostrap.Utility.ScreenColorEffect.ApplyConfigured();
        }
        ...
        private async Task<string> GetServerLocationAsync(string ip, CancellationToken token)
        {
            try
            {
				...
				var dc = await Xenostrap.Integrations.XenostrapMatchmaker.LookupUnknownIpAsync(ip, token);
                if (dc == null || string.IsNullOrEmpty(dc.City)) return "Location: Unknown";
                string country = Xenostrap.Integrations.XenostrapMatchmaker.CountryToDisplayName(dc.Country);
