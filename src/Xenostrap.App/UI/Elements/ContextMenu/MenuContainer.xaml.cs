...
using Xenostrap.UI.Chat;
using Xenostrap.UI.Elements.Base;
using Xenostrap.UI.Elements.Crosshair;
using Xenostrap.UI.Elements.Overlay;
...
public partial class MenuContainer : WpfUiWindow
{
    ...
    private void LoadFlags()
    {
        try
        {
			...
			int totalFlags = Xenostrap.Utility.JsonFile.Deserialize<Dictionary<string, JsonElement>>(path, JsonOptions.Tolerant, 16777216).Count;
    ...
    public MenuContainer(Watcher watcher)
    {
        ...
        if (Xenostrap.Integrations.FrameGeneration.FrameGenSettings.ModeIndex > 0 || App.Settings.Prop.FrameGenResumeIndex > 0)
        {
            FrameGenParentMenuItem.Visibility = Visibility.Visible;
            FrameGenMenuItem.IsChecked = Xenostrap.Integrations.FrameGeneration.FrameGenSettings.ModeIndex > 0;
        ...
        VersionTextBlock.Text = "Xenostrap v" + App.Version;
    ...
    private async Task<(string Name, BitmapSource? Icon)> LoadGamePresentationAsync(ActivityData data, CancellationToken token)
    {
        ...
        try
        {
            gameIcon = await Xenostrap.Utility.AppImage.LoadAsync(iconUrl, 128, token);
        }
        catch (OperationCanceledException)
        {
            throw;
    ...
    private async Task<ServerInfo?> FetchClosestServerAsync(long placeId, CancellationToken token)
    {
        ...
        ServerListResponse? servers = JsonSerializer.Deserialize<ServerListResponse>(await Xenostrap.Utility.Http.ReadStringBoundedAsync(response.Content, 4 * 1024 * 1024, timeoutCts.Token), JsonOptions.Tolerant);
    ...
    private void JoinServer(string serverId, long placeId)
    {
        try
        {
            ...
            if (successor == null)
                throw new InvalidOperationException("Xenostrap did not start.");
    ...
    private async Task ShowJoinNotification(ActivityData data, Task<(string Name, BitmapSource? Icon)> presentationTask, CancellationToken token)
    {
        ...
        try
        {
            flagImage = await Xenostrap.Utility.CountryFlag.GetImageAsync(data.ServerCountryCode, token).WaitAsync(NotificationEnrichTimeout, token);
        }
        catch (OperationCanceledException)
        {
            return;
    ...
    private void RefreshQuestProgress()
    {
        if (QuestExpander == null || QuestProgressList == null)
            return;
        Xenostrap.Models.QuestProgressSnapshot? snapshot = Xenostrap.Integrations.QuestTracker.Progress;
    ...
    public void ApplyBackdrop()
    {
        if (base.ContextMenu?.IsOpen == true)
            Xenostrap.UI.WindowBackdrop.ApplyContextMenu(base.ContextMenu);
    }
    ...
    private void FrameGenMenuItem_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            Xenostrap.Integrations.FrameGeneration.FrameGenManager.SetMode(1);
            FrameGenMenuItem.IsChecked = Xenostrap.Integrations.FrameGeneration.FrameGenSettings.ModeIndex > 0;
        }
        else
        {
            App.Settings.Prop.FrameGenResumeIndex = 1;
            Xenostrap.Integrations.FrameGeneration.FrameGenManager.SetMode(0);
        }
    ...
    private void CantSeeOverlaysMenuItem_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            Xenostrap.Integrations.Overlays.OverlayDiagnostics.RaiseOverlayWindows();
            string report = Xenostrap.Integrations.Overlays.OverlayDiagnostics.BuildReport();
	...
	{
		if (_activityWatcher == null || _closed)
			return;
		try
		{
			if (_musicPlayerWindow == null)
			{
				...
				_musicPlayerWindow.Closed += ChildWindow_Closed;
			}
		...
		{
			App.Logger.WriteException("MenuContainer::MusicPlayerMenuItem_Click", ex);
			if (_musicPlayerWindow != null)
				_musicPlayerWindow.Closed -= ChildWindow_Closed;
			_musicPlayerWindow = null;
		}
