...
using Xenostrap.UI.Elements.Settings.Pages;
using Xenostrap.UI;
...
public partial class MainWindow : WpfUiWindow, INavigationWindow
{
    ...
    {
        ["HomePage"] = ("Home", "On the Xenostrap home screen"),
        ...
        ["ReleasesPage"] = ("Releases", "Xenostrap release history"),
        ...
        ["NewsPage"] = ("News", "What's new in Xenostrap"),
    ...
    private readonly Xenostrap.Utility.WebsiteNotificationRealtime _notificationRealtime = new Xenostrap.Utility.WebsiteNotificationRealtime();
    ...
    private static readonly List<TabOptionDefinition> TabOptionRegistry = new List<TabOptionDefinition>
    {
        ...
        Toggle("Background Snow", "Background Snow", "Adds snow to Xenostrap's background.", () => App.Settings.Prop.SnowWOWSOCOOLWpfSnowbtw, delegate(bool v)
        {
        ...
        SliderOption("CPU Core Limit", "Xenostrap CPU core limit", "Limits the processor affinity of Xenostrap itself. Leave it at the maximum unless you need to reserve cores for another app.", 1.0, ProcessorCount, 1.0, () => App.Settings.Prop.CpuCoreLimit, delegate(double v)
        {
		...
		SliderOption("Max Concurrent Downloads", "Max Concurrent Downloads", "How many files Xenostrap downloads at once when installing Roblox.", 1.0, 32.0, 1.0, () => App.Settings.Prop.MaxConcurrentDownloads, delegate(double v)
        {
    ...
    private Xenostrap.Models.Persistable.WindowState _state => App.State.Prop.SettingsWindow;
    ...
    {
        base.ApplyTheme();
        Xenostrap.UI.WindowBackdrop.ApplyMainWindow(this);
    }
    ...
    private void ApplyThemeBackground()
    {
        ...
        Brush surface = Xenostrap.UI.WindowBackdrop.CreateSurfaceBrush(this);
    ...
    {
        if (!Xenostrap.Utility.Platform.IsWindows || BackgroundLayer == null)
        {
            return;
        }
        try
    ...
    public MainWindow(bool showAlreadyRunningWarning)
    {
        ...
        InitializeComponent();
        SoberNavItem.Visibility = Xenostrap.Utility.Platform.IsLinux ? Visibility.Visible : Visibility.Collapsed;
    ...
    private void SaveTabsStructure()
    {
        ...
        try
        {
            ...
            Xenostrap.Utility.JsonFile.SerializeAtomic(TabsConfigPath, tabs, Xenostrap.Utility.JsonOptions.Indented);
        }
        catch (Exception ex)
        {
    ...
    private void LoadTabsStructure()
    {
        ...
        try
        {
            List<TabBlueprint> list = Xenostrap.Utility.JsonFile.Deserialize<List<TabBlueprint>>(TabsConfigPath, Xenostrap.Utility.JsonOptions.Tolerant, 4194304);
	...
	private static void RevealSearchContainers(Page page, TopSearchEntry entry)
	{
		...
		foreach (FrameworkElement element in EnumerateSearchElements(page))
		{
			...
			else if (element is Xenostrap.UI.Elements.Controls.Expander expander)
			{
    ...
    private static void RevealTopSearchTarget(FrameworkElement target, Page page)
    {
        ...
        {
            if (current is Xenostrap.UI.Elements.Controls.Expander expander)
            {
                expander.IsExpanded = true;
            }
    ...
    public async Task SetBackgroundImage(string? path, bool loop = true)
    {
        ...
        if (flag)
        {
            ...
            BitmapSource? bitmap = await Task.Run(() => Xenostrap.Utility.SafeImaging.FromFile(path, num));
            if (!IsCurrentBackgroundOperation(generation))
            {
                return;
            }
            ...
            case ".gif":
                {
                    ...
                    {
                        BackgroundImage.Source = Xenostrap.Utility.SafeImaging.FromFile(path);
                    }
                    BackgroundImage.Visibility = Visibility.Visible;
                    if (BackgroundMedia != null)
                    {
    ...
    {
        _discordClient = new DiscordRpcClient("1539279631264518205");
    ...
    {
        UpdateDiscordPresence();
        if (RootFrame?.Content is Xenostrap.UI.Elements.Settings.Pages.NewsPage)
        {
            ClearNewsBadge();
        }
    }
    ...
    private async Task FetchRpcAvatarAsync()
    {
        try
        {
            string cookie = Xenostrap.Integrations.RobloxCookie.Get();
            ...
            using (var request = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Get, "https://users.roblox.com/v1/users/authenticated"))
            {
                ...
                string text = await Xenostrap.Utility.Http.ReadStringBoundedAsync(response.Content, 200000, cts.Token).ConfigureAwait(continueOnCapturedContext: false);
            ...
            using (var cts2 = CancellationTokenSource.CreateLinkedTokenSource(_lifetimeCts.Token))
            {
                ...
                string thumbText = await Xenostrap.Utility.Http.ReadStringBoundedAsync(thumbResponse.Content, 200000, cts2.Token).ConfigureAwait(continueOnCapturedContext: false);
    ...
    private async Task CheckForNewNewsAsync()
    {
        try
        {
            ...
            string text = await Xenostrap.Utility.Http.ReadStringBoundedAsync(response.Content, 2000000, cts.Token).ConfigureAwait(continueOnCapturedContext: false);
            string key = "";
            using (var doc = System.Text.Json.JsonDocument.Parse(text))
            {
    ...
    private void UpdateDiscordPresence()
    {
        ...
        {
            details = text2;
            state = "Exploring Xenostrap";
        }
        else
        {
            details = "Idle";
            state = "Configuring Xenostrap";
        }
        ...
        string largeImageText = (string.IsNullOrWhiteSpace(text3) ? "Xenostrap" : ("Xenostrap v" + text3));
        try
        {
            _discordClient.SetPresence(new DiscordRPC.RichPresence
            {
                ...
                Assets = new Assets
                {
                    LargeImageKey = "https://xenostrapp.pages.dev/assets/img/xenostrap.png",
                ...
                Buttons = new DiscordRPC.Button[2]
                {
                    ...
                    {
                        Label = "Discord",
                        Url = "https://discord.gg/Ay7GhEvxYZ"
                    },
                    new DiscordRPC.Button
                    {
                        Label = "Github",
                        Url = Xenostrap.Utility.GitHubCache.PreferredRepository
                    }
                }
            });
    ...
    private async void MainWindow_Loaded(object? sender, RoutedEventArgs e)
    {
        ...
        Microsoft.Win32.SystemEvents.DisplaySettingsChanged += OnDisplaySettingsChanged;
        Xenostrap.Utility.WebsiteNotifications.UnreadChanged -= OnNotificationsUnreadChanged;
        Xenostrap.Utility.WebsiteNotifications.UnreadChanged += OnNotificationsUnreadChanged;
        _notificationRealtime.Start();
        UpdateNotificationsButtonState(true);
        _ = RefreshNotificationBadgeAsync(true);
    ...
    {
        if (!Xenostrap.Utility.WebsiteAuth.IsSignedIn())
            return;
        if (AccountPopup != null)
            AccountPopup.IsOpen = false;
        try
    ...
    {
        bool signedIn = Xenostrap.Utility.WebsiteAuth.IsSignedIn();
    ...
    {
        if (_isClosed || !Xenostrap.Utility.WebsiteAuth.IsSignedIn())
            return;
        try
        {
    ...
    private async Task RefreshNotificationBadgeAsync(bool force = false)
    {
        ...
        try
        {
            await Xenostrap.Utility.WebsiteNotifications.GetAsync(_lifetimeCts.Token).ConfigureAwait(false);
        }
        finally
        {
    ...
    private async void AccountSignOut_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            string? token = Xenostrap.Utility.WebsiteAuth.GetToken();
            Xenostrap.Utility.WebsiteAuth.Clear();
            RefreshAccountUi();
            UpdateAccountButtonsEnabled();
    ...
    {
        string activeAccount = Xenostrap.Utility.WebsiteAuth.GetActiveId() ?? "";
        try
        {
            Xenostrap.Utility.WebsiteBorderData data = await Xenostrap.Utility.WebsiteBorderRenderer.FetchActiveAsync(20.0, 30.0).ConfigureAwait(continueOnCapturedContext: false);
            if (_isClosed || activeAccount != (Xenostrap.Utility.WebsiteAuth.GetActiveId() ?? ""))
                return;
            await ((DispatcherObject)this).Dispatcher.InvokeAsync((Action)delegate
            {
                if (_isClosed || activeAccount != (Xenostrap.Utility.WebsiteAuth.GetActiveId() ?? ""))
                ...
                try
                {
                    if (AccountAvatarRing != null)
                    {
                        System.Windows.Media.Brush ringBrush = (data != null && !string.IsNullOrEmpty(data.GradientBorderKey))
                            ? Xenostrap.Utility.GradientProfileBorder.ParseBorder(data.GradientBorderKey)
                            : null;
    ...
    private async Task LoadAvatarAsync(long userId)
    {
        try
        {
            ...
            using JsonDocument doc = JsonDocument.Parse(await Xenostrap.Utility.Http.GetString(requestUri, token).ConfigureAwait(false));
            ...
            BitmapSource? bitmap = await Xenostrap.Utility.AppImage.LoadAsync(text, 150, token).ConfigureAwait(false);
            if (bitmap == null)
            {
                return;
            }
    ...
    protected override void OnActivated(EventArgs e)
    {
        ...
        try
        {
            ...
            if (Xenostrap.Utility.WebsiteAuth.IsSignedIn() && Environment.TickCount64 - _lastBorderRefreshTicks > 30000L)
            {
    ...
    private void UpdateButtonContent()
    {
        ...
        string content;
        if (base.DataContext is MainWindowViewModel clientVm && !string.IsNullOrEmpty(clientVm.SelectedLaunchClient) && Xenostrap.Utility.ClassicClients.IsClientInstalled(clientVm.SelectedLaunchClient))
        {
    ...
    private static bool IsLaunchTargetInstalled(bool studio)
    {
        try
        {
            Xenostrap.AppData.IAppData appData = (studio ? ((Xenostrap.AppData.IAppData)new Xenostrap.AppData.RobloxStudioData()) : ((Xenostrap.AppData.IAppData)new Xenostrap.AppData.RobloxPlayerData()));
	...
	private async void RestartNotificationAction_Click(object sender, RoutedEventArgs e)
	{
		...
		{
			App.Logger.WriteException("MainWindow::RestartNotification", ex);
			ShowRestartNotificationFailure("Xenostrap could not restart. Check the logs and try again.");
		}
	}

	private void RestartXenostrapFromSettings()
	{
		string executable = "";
		try
		{
		...
		if (string.IsNullOrWhiteSpace(executable) || !File.Exists(executable))
		{
			throw new FileNotFoundException("The Xenostrap executable could not be found");
		}
		...
		if (process == null)
		{
			throw new InvalidOperationException("The Xenostrap restart process did not start");
		}

		App.Logger.WriteLine("MainWindow::RestartNotification", "Restarting Xenostrap from settings");
		RestartNotificationService.ClearAll();
    ...
    private void WpfUiWindow_Closed(object sender, EventArgs e)
    {
        ...
        Xenostrap.Utility.DynamicRenderSystem.ClearCache();
    ...
    private void PopulateLaunchTargets()
    {
        ...
        {
            if (!string.IsNullOrEmpty(vm.SelectedLaunchClient) && Xenostrap.Utility.ClassicClients.IsClientInstalled(vm.SelectedLaunchClient))
            {
                currentKind = "client";
        ...
        try
        {
            foreach (string code in Xenostrap.Utility.ClassicClients.ListInstalledClients())
            {
                var config = Xenostrap.Utility.ClassicClients.GetInstalledConfig(code);
