...
namespace Xenostrap.UI.Elements.Settings.Pages
{
    public partial class HomePage : Page
    {
        ...
        private async void OnViewModelPropertyChanged(object? sender, PropertyChangedEventArgs e)
        {
            try
            {
                if (e.PropertyName == nameof(HomePageViewModel.WebsiteAvatarUrl))
                {
                    ...
                    var bitmap = await Xenostrap.Utility.AppImage.LoadAsync(imageUrl, 256, lifetimeCts.Token).ConfigureAwait(false);
        ...
        private void EditProfile_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var win = new Xenostrap.UI.Elements.ContextMenu.ProfileEditorWindow
                {
