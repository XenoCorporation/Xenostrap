    ...
    public partial class GamePage
    {
        ...
        private async Task LoadGameAsync(CancellationToken token)
        {
            try
            {
                if (_universeId == 0)
                {
					string uniJson = await Xenostrap.Utility.Http.GetString(
        ...
        private async Task LoadDetailsAsync(CancellationToken token)
        {
            try
            {
				string json = await Xenostrap.Utility.Http.GetString(
        ...
        private async Task LoadBannerAsync(CancellationToken token)
        {
            try
            {
				...
				var bmp = await Xenostrap.Utility.GradientWebsite.LoadBannerImageAsync(imageUrl, token).ConfigureAwait(false)
					?? await Xenostrap.Utility.AppImage.LoadAsync(imageUrl, 960, token).ConfigureAwait(false);
				if (bmp == null)
					return;
				token.ThrowIfCancellationRequested();
				await Dispatcher.InvokeAsync(() =>
