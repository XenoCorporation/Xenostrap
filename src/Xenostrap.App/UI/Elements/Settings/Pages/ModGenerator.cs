...
namespace Xenostrap.UI.Elements.Settings.Pages
{
    public static class ModGenerator
    {
        ...
        public static void RecolorAllPngs(
            ...
            bool colorVoiceChat)
        {
            if (string.IsNullOrEmpty(xenostrapTemp) || !Directory.Exists(xenostrapTemp))
                return;

            foreach (string path in Directory.EnumerateFiles(xenostrapTemp, "*.png", SearchOption.AllDirectories))
            {
            ...
            ApplyCustomImage(xenostrapTemp, customLogoPath, "logo");
            ApplyCustomImage(xenostrapTemp, customSpinnerPath, "spinner");
        }
        ...
        private static void ApplyCustomImage(string xenostrapTemp, string? customPath, string keyword)
        {
            if (string.IsNullOrEmpty(customPath) || !File.Exists(customPath))
                return;
