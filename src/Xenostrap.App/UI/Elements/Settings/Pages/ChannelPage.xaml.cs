...
public partial class ChannelPage : UiPage{
	...
	private void ApplyNow_Click(object sender, RoutedEventArgs e)
	{
		try
		{
			...
			foreach (string item in (from d in Directory.GetDirectories(folderPath)
				where d.EndsWith("strap", StringComparison.OrdinalIgnoreCase) && !d.EndsWith("Xenostrap", StringComparison.OrdinalIgnoreCase)
				select d).ToList())
			{
			...
			Frontend.ShowMessageBox("Xenostrap Settings/Mods Synced");
		}
		catch (Exception ex)
		{
	...
	private async void Check_Click(object sender, RoutedEventArgs e)
	{
		...
		try
		{
				...
				if (newer)
			{
				...
				if (!await Xenostrap.Extensions.GithubUpdater.DownloadAndInstallUpdate(release.TagName))
				{
			...
			{
				Frontend.ShowMessageBox("You are already running the latest version of Xenostrap (" + currentVersion + ").");
			}
		}
		catch (OperationCanceledException)
		{
	...
	private void ResetSettingsButton_Click(object sender, RoutedEventArgs e)
	{
		...
		if (Frontend.ShowMessageBox("Erase all Xenostrap settings, FastFlags, modifications, accounts, themes, caches, and saved data, then return to onboarding? This cannot be undone.", MessageBoxImage.Warning, MessageBoxButton.YesNo, MessageBoxResult.No) != MessageBoxResult.Yes)
		{
			return;
		}
		...
		try
		{
			...
			}) ?? throw new InvalidOperationException("Xenostrap could not start the factory reset");
	...
	private void UninstallButton_Click(object sender, RoutedEventArgs e)
	{
		...
		{
			Xenostrap.Installer.DoUninstall(uninstallerDialog.KeepData);
