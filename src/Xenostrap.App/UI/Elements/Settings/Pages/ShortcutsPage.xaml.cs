...
public partial class ShortcutsPage : UiPage{
	...
	private static readonly HttpClient _httpClient = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromSeconds(15));

	private static readonly HttpClient _noRedirectClient = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromSeconds(15), handler => handler.AllowAutoRedirect = false);
	...
	private async void BtnCreateShortcut_Click(object sender, RoutedEventArgs e)
	{
		...
		{
			App.Logger.WriteLine("Shortcuts::CreateShortcut", "Game icon lookup failed, using the Xenostrap icon instead");
		...
		try
		{
			...
			Xenostrap.Utility.Shortcut.Create(executable, arguments, shortcutPath, iconPath);
			if (!File.Exists(shortcutPath))
			{
				Frontend.ShowMessageBox("Failed to create shortcut.\n\nWindows did not let Xenostrap write to the desktop.");
				return;
			}
	...
	private async Task<string?> FetchGameIconUrlAsync(string gameId)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			try
			{
				...
				string json = await Xenostrap.Utility.Http.ReadStringBoundedAsync(response.Content, 262144, requestCts.Token).ConfigureAwait(false);
	...
	private async Task<string> DownloadAndForceIcoAsync(string? imageUrl, string baseName)
	{
		try
		{
			...
			response.EnsureSuccessStatusCode();
			byte[] imageBytes = await Xenostrap.Utility.Http.ReadBytesBoundedAsync(response.Content, 4 * 1024 * 1024, imageCts.Token).ConfigureAwait(false);
