...
public sealed class QuestItem : System.ComponentModel.INotifyPropertyChanged
{
    ...
    public Xenostrap.Models.QuestLiveSession? Session { get; set; }
...
public partial class QuestsPage : UiPage
{
    ...
    private Xenostrap.Models.QuestLiveSession? _session;
    ...
    private async Task LoadAsync()
    {
        ...
        try
        {
            ...
            string body = await Xenostrap.Utility.Http.ReadStringBoundedAsync(response.Content, 4 * 1024 * 1024, timeout.Token).ConfigureAwait(true);
    ...
    private void Apply(string body)
    {
        ...
        Xenostrap.Models.QuestLiveSession? live = null;
        ...
        try
        {
            ...
            {
                live = Xenostrap.Models.QuestLiveSession.FromResponse(root);
    ...
    private async Task<string?> GetAsync(string url, bool authenticated, CancellationToken token)
    {
        ...
        if (!response.IsSuccessStatusCode)
            return null;
        return await Xenostrap.Utility.Http.ReadStringBoundedAsync(response.Content, 4 * 1024 * 1024, token).ConfigureAwait(true);
    }
    ...
    private async Task VoteAsync(QuestItem item, int value)
    {
        ...
        try
        {
            ...
            using HttpResponseMessage response = await App.HttpClient.SendAsync(request, HttpCompletionOption.ResponseHeadersRead, timeout.Token).ConfigureAwait(true);
            string body = await Xenostrap.Utility.Http.ReadStringBoundedAsync(response.Content, 1024 * 1024, timeout.Token).ConfigureAwait(true);
            if (!response.IsSuccessStatusCode)
                return;
            using JsonDocument document = JsonDocument.Parse(body);
