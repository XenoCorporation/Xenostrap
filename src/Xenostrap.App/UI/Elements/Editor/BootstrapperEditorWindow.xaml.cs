...
using Xenostrap.UI.ViewModels.Editor;
using Wpf.Ui.Common;
using Wpf.Ui.Controls;

namespace Xenostrap.UI.Elements.Editor;
...
public partial class BootstrapperEditorWindow : WpfUiWindow{
	...
	private static void RestoreMainWindow()
	{
		try
		{
			var main = System.Windows.Application.Current?.Windows
				.OfType<Xenostrap.UI.Elements.Settings.MainWindow>()
				.FirstOrDefault();
	...
	private Xenostrap.UI.Elements.Bootstrapper.CustomDialog? _previewDialog;
	...
	private void RenderPreview(bool force = false)
	{
		...
		Xenostrap.UI.Elements.Bootstrapper.CustomDialog? dialog = null;

		try
		{
			dialog = new Xenostrap.UI.Elements.Bootstrapper.CustomDialog(true);
		...
		try
		{
			...
			Xenostrap.Models.BackdropType backdrop = Xenostrap.UI.WindowBackdrop.ResolveFor(dialog);
			bool glass = backdrop != Xenostrap.Models.BackdropType.None;
			...
			if (glass)
			{
				...
				{
					Background = new System.Windows.Media.SolidColorBrush(Xenostrap.UI.WindowBackdrop.CreateSurfaceColor(backdrop))
				};
	...
	private static double BlurFor(Xenostrap.Models.BackdropType backdrop)
	{
		return backdrop switch
		{
			Xenostrap.Models.BackdropType.Mica => 100,
			Xenostrap.Models.BackdropType.MicaAlt => 110,
			Xenostrap.Models.BackdropType.Acrylic => 42,
			Xenostrap.Models.BackdropType.Aero => 42,
			_ => 60
		};
	}

	private static UIElement BuildBackdropLayer(Xenostrap.Models.BackdropType backdrop)
	{
	...
	private static CornerRadius CornerFor(Xenostrap.UI.Elements.Bootstrapper.CustomDialog dialog)
	{
	...
	private static string HintFor(string message)
	{
		...
		if (lower.Contains("root"))
			return "The outer tag has to be XenostrapCustomBootstrapper.";
	...
	{
		var dialog = new Xenostrap.UI.Elements.Dialogs.TextInputDialog(title, initial);
		dialog.Owner = this;
		dialog.ShowDialog();
	...
	private static string StarterContentFor(string name)
	{
		...
		if (extension == ".js")
			return "window.xenostrap.onUpdate(function (state) {\r\n});\r\n";
