...
public static class RoundedWindowChrome
{
	...
	{
		if (_installed || Xenostrap.Utility.Platform.IsWindows)
		{
			return;
		}
		_installed = true;
	...
	{
		if (window == null || Xenostrap.Utility.Platform.IsWindows)
		{
			return;
		}
		try
	...
	private static void ApplyStartupLocation(Window window)
	{
		try
		{
			...
			{
				(double screenWidth, double screenHeight) = Xenostrap.Utility.ScreenMetrics.GetPrimary();
				left = (screenWidth - width) / 2.0;
				top = (screenHeight - height) / 2.0;
			}

			(double boundsWidth, double boundsHeight) = Xenostrap.Utility.ScreenMetrics.GetPrimary();
