...
using Xenostrap.UI.Elements.Installer;
...
public static class LaunchHandler
{
	...
	public static void ProcessLaunchArgs()
	{
		...
		if (App.LaunchSettings.WindowAuditFlag.Active)
		{
			...
			try
			{
				Xenostrap.Utility.WindowAudit.Run();
			}
		...
		if (App.LaunchSettings.NvApplyFlag.Active)
		{
			...
			try
			{
				applied = Xenostrap.Integrations.NvidiaProfileManager.ApplyStagedFile(staged);
			}
	...
	public static void LaunchInstaller()
	{
		...
		try
		{
			...
			else
			{
				...
				Xenostrap.UI.Elements.Installer.MainWindow mainWindow = new Xenostrap.UI.Elements.Installer.MainWindow();
	...
	public static void LaunchThemeInstall(string? raw)
	{
		...
		Task.Run(async delegate
		{
			try
			{
				string folder = await Xenostrap.Integrations.BootstrapperThemes.InstallFromWebsiteAsync(themeId);
	...
	{
		if (Xenostrap.Utility.Platform.IsWindows && !ProcessElevation.IsAdministrator())
		{
	...
	public static void LaunchSettings()
	{
		...
		if (interProcessLock.IsAcquired)
		{
			new Xenostrap.UI.Elements.Settings.MainWindow(Process.GetProcessesByName("Xenostrap").Length > 1).ShowDialog();
			return;
		}
	...
	public static void LaunchRoblox(LaunchMode launchMode)
	{
		...
		if (Xenostrap.Utility.Platform.SupportsWindowsClient && !File.Exists(Path.Combine(Paths.System, "mfplat.dll")))
		{
		...
		bool flag2 = string.Equals(Environment.GetEnvironmentVariable("XENOSTRAP_FORCE_NATIVE"), "1", StringComparison.Ordinal);
		if (!flag2)
		{
			CloseOtherInstances();
		}
	...
	private static async Task LaunchPortableRuntimeAsync(LaunchMode launchMode)
	{
		...
		try
		{
			...
			Xenostrap.Platform.RuntimeKind runtimeKind = launchMode == LaunchMode.Player
				? Xenostrap.Platform.RuntimeKind.Player
				: Xenostrap.Platform.RuntimeKind.Studio;
			string launchTarget = App.LaunchSettings.RobloxLaunchArgs;
			if (string.IsNullOrWhiteSpace(launchTarget))
			{
				launchTarget = runtimeKind == Xenostrap.Platform.RuntimeKind.Player
					? "roblox://experiences/start"
					: "roblox-studio://launch";
			}
			string rewrittenTarget = await Bootstrapper.RewriteXenostrapMatchmakerBeforeDispatchAsync(
				launchTarget,
				launchMode,
			...
			{
				Xenostrap.Platform.IRobloxRuntimeProvider provider = runtimeKind == Xenostrap.Platform.RuntimeKind.Player
					? host.PlayerRuntime
					: host.StudioRuntime;
				Xenostrap.Platform.RuntimeInstallation installation = await provider.FindInstallationAsync();
				...
				Xenostrap.Platform.Linux.LinuxRuntimeConfiguration configuration = Xenostrap.Platform.Linux.LinuxRuntimeConfiguration.CreateDefault(Paths.Mods, host.Processes);
				Xenostrap.Platform.OperationResult prepared = await configuration.PrepareAsync(
					installation,
			...
			Xenostrap.Core.RuntimeLaunchCoordinator coordinator = new(host.PlayerRuntime, host.StudioRuntime);
			Xenostrap.Platform.OperationResult<Xenostrap.Platform.LaunchSession> result = await coordinator.LaunchAsync(runtimeKind, launchTarget);
	...
	private static void CloseOtherInstances()
	{
		try
		{
			...
			Process[] processesByName = Process.GetProcessesByName("Xenostrap");
			foreach (Process process in processesByName)
			{
				try
				{
					...
					App.Logger.WriteLine("LaunchHandler::CloseOtherInstances", $"Closing other {"Xenostrap"} window (pid {process.Id})");
