...
namespace Xenostrap.RobloxInterfaces;
...
public static class Deployment
{
	...
	static Deployment()
	{
		...
		SharedHttp = Xenostrap.Utility.VpnHttpClient.Create(TimeSpan.FromSeconds(30L));
		SharedHttp.DefaultRequestHeaders.TryAddWithoutValidation("User-Agent", "XenostrapUpdater/2.0");
	...
	private static async Task<T> SafeGetJson<T>(string channel, string binaryType, CancellationToken cancellationToken)
	{
		...
		foreach (string host in ClientSettingsHosts)
		{
			...
			try
			{
				...
				string text = await Xenostrap.Utility.Http.ReadStringBoundedAsync(response.Content, MaxVersionResponseBytes, cancellationToken).ConfigureAwait(false);
	...
	private static async Task<string?> ProbeAsync(string url, CancellationToken token)
	{
		try
		{
			...
			return ((await Xenostrap.Utility.Http.ReadStringBoundedAsync(resp.Content, 1024, token).ConfigureAwait(false)).Trim() == VersionStudioHash) ? url : null;
		}
		catch
		{
			return null;
