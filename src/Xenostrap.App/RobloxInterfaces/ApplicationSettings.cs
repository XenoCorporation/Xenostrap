...
public class ApplicationSettings
{
	...
	private static async Task<SourceResult> FetchSourceAsync(string source, string path, string logIdent, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			{
				string errorBody = await Xenostrap.Utility.Http.ReadStringBoundedAsync(response.Content, MaxErrorBytes, cancellationToken).ConfigureAwait(false);
			...
			string body = await Xenostrap.Utility.Http.ReadStringBoundedAsync(response.Content, MaxResponseBytes, cancellationToken).ConfigureAwait(false);
