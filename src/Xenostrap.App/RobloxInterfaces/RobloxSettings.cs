...
namespace Xenostrap
{
    public static class RobloxSettings
    {
        ...
        public static void SetCap(int fps)
        {
            try
            {
                string value = fps.ToString();
                if (!File.Exists(SettingsPath))
                {
                    var newDoc = new XDocument(
                        new XElement("robloxSettings",
                            new XElement("int", new XAttribute("name", "FramerateCap"), value)
                        )
                    );
                    Directory.CreateDirectory(Path.GetDirectoryName(SettingsPath)!);
                    newDoc.Save(SettingsPath);
                ...
                if (fpsElement != null)
                    fpsElement.Value = value;
                else
                    doc.Root?.Add(new XElement("int", new XAttribute("name", "FramerateCap"), value));
