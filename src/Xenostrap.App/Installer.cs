...
internal partial class Installer
{
	...
	public string InstallLocation = Path.Combine(Paths.LocalAppData, "Xenostrap");
	...
	public bool XenostrapRPCReal = true;

	public bool IsImplicitInstall;

	private static string DesktopShortcut => Path.Combine(Paths.Desktop, "Xenostrap.lnk");

	private static string StartMenuShortcut => Path.Combine(Paths.WindowsStartMenu, "Xenostrap.lnk");
	...
	public void DoInstall()
	{
		...
		Xenostrap.Integrations.Youtube.YoutubeToolchain.PreinstallAsync();
		if (!IsImplicitInstall && !string.Equals(Paths.Process, Paths.Application, StringComparison.InvariantCultureIgnoreCase))
		{
			...
			try
			{
				...
				Xenostrap.Utility.InstallRecord.MakeExecutable(Paths.Application);
			}
			catch (Exception ex)
			{
		...
		Xenostrap.Utility.InstallRecord.Write(Paths.Base);
		if (Xenostrap.Utility.Platform.SupportsRegistry)
		{
			TrySafe("uninstall registry entry", delegate
			{
				using RegistryKey registryKey = Registry.CurrentUser.CreateSubKey("Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Xenostrap");
				registryKey.SetValueSafe("DisplayIcon", Paths.Application + ",0");
				registryKey.SetValueSafe("DisplayName", "Xenostrap");
				...
				registryKey.SetValueSafe("Publisher", "Xenostrap");
				...
				registryKey.SetValueSafe("URLInfoAbout", Xenostrap.Utility.GitHubCache.PreferredRepository + "/issues/new");
				registryKey.SetValueSafe("URLUpdateInfo", Xenostrap.Utility.GitHubCache.PreferredRepository + "/releases");
			});
			WindowsRegistry.RegisterApis();
			WindowsRegistry.RegisterPlayer();
			WindowsRegistry.RegisterStudioProtocol(Paths.Application, "-studio \"%1\"");
			WindowsRegistry.RegisterXenostrap();
		}
		if (Xenostrap.Utility.Platform.IsWindows)
		{
			if (CreateDesktopShortcuts)
			{
				TrySafe("desktop shortcut", () => Xenostrap.Utility.Shortcut.Create(Paths.Application, "", DesktopShortcut));
			}
			if (CreateStartMenuShortcuts)
			{
				TrySafe("start menu shortcut", () => Xenostrap.Utility.Shortcut.Create(Paths.Application, "", StartMenuShortcut));
			}
			TrySafe("function shortcuts", CreateFunctionShortcuts);
		}
		else
		{
			TrySafe("desktop entry", () => Xenostrap.Utility.LinuxDesktopEntry.Install(Paths.Application));
		}
	...
	private static void ApplyPendingAuth()
	{
		...
		try
		{
			Xenostrap.Utility.WebsiteAuth.AddOrUpdateAccount(PendingAuthToken, PendingAuthId, PendingAuthLabel, PendingAuthAvatar);
			...
			Xenostrap.Utility.WebsiteAuth.Notify();
		}
		catch (Exception ex)
		{
	...
	private void CreateFunctionShortcuts()
	{
		...
		try
		{
			Xenostrap.Models.SettingTasks.ExtractIconsTask.ExtractAll(overwrite: true);
		}
		catch (Exception ex)
		{
	...
	private static void TryCreateShortcut(bool wanted, string flags, string name)
	{
		...
		try
		{
			Xenostrap.Utility.Shortcut.Create(Paths.Application, flags, Path.Combine(Paths.Desktop, name + ".lnk"));
		}
		catch (Exception ex)
		{
	...
	public bool CheckInstallLocation()
	{
		...
		{
			if (!IsImplicitInstall && !InstallLocation.EndsWith("Xenostrap", StringComparison.InvariantCultureIgnoreCase) && Directory.Exists(InstallLocation) && Directory.EnumerateFileSystemEntries(InstallLocation).Any())
			{
				string text = Path.Combine(InstallLocation, "Xenostrap");
			...
			try
			{
				string path = Path.Combine(InstallLocation, "XenostrapWriteTest.txt");
				Directory.CreateDirectory(InstallLocation);
	...
	private static readonly string[] PreservedFolders = ["Config", "XenostrapMods", "Themes", "Modifications"];
	...
	public static void DoUninstall(bool keepData)
	{
		...
		Xenostrap.Integrations.AssetProxy.AssetProxyServer.Stop();
		...
		Xenostrap.Utility.LinuxDesktopEntry.Remove();
		Xenostrap.Utility.InstallRecord.Delete();
		if (Xenostrap.Utility.Platform.IsWindows)
		{
			...
			TrySafe("registry Software\\Xenostrap", delegate
			{
				Registry.CurrentUser.DeleteSubKeyTree("Software\\Xenostrap", throwOnMissingSubKey: false);
			});
		}
		List<Action> list2 =
		[
		...
		list2.Add(delegate
		{
			ForceDeleteDirectory(Path.Combine(Path.GetTempPath(), "Xenostrap"));
		});

		list2.Add(delegate
		{
			if (Xenostrap.Utility.Platform.IsWindows)
				Registry.CurrentUser.DeleteSubKeyTree("Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Xenostrap", throwOnMissingSubKey: false);
		});
	...
	private static void ScheduleDeferredCleanup(string target, bool deleteDirectory, string logSource)
	{
		try
		{
			...
			string markerName = ".Xenostrap.cleanup." + Guid.NewGuid().ToString("N");
			...
			string helperDirectory = Path.Combine(Path.GetTempPath(), "XenostrapCleanup");
			Directory.CreateDirectory(helperDirectory);
			CleanupOldHelpers(helperDirectory);
			string helperPath = Path.Combine(helperDirectory, "XenostrapCleanup." + Guid.NewGuid().ToString("N") + Path.GetExtension(Paths.Process));
	...
	internal static async Task RunDeferredCleanupAsync(string? encodedPayload)
	{
		...
		try
		{
			...
			if (!IsOwnedInstallRoot(root) || Path.GetFileName(payload.MarkerName) != payload.MarkerName || !payload.MarkerName.StartsWith(".Xenostrap.cleanup.", StringComparison.Ordinal))
				return;
			if (!payload.DeleteDirectory)
			{
				string fileName = Path.GetFileName(fullTarget);
				if (!string.Equals(fileName, "Xenostrap.exe", StringComparison.OrdinalIgnoreCase) && !string.Equals(fileName, "Xenostrap", StringComparison.OrdinalIgnoreCase))
					return;
			}
	...
	private static bool IsOwnedInstallRoot(string root)
	{
		...
		if (string.Equals(fullRoot, Path.GetPathRoot(fullRoot)?.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar), StringComparison.OrdinalIgnoreCase))
			return false;
		return File.Exists(Path.Combine(fullRoot, "Xenostrap.exe")) || File.Exists(Path.Combine(fullRoot, "Xenostrap"));
	}

	private static void CleanupOldHelpers(string directory)
	{
		foreach (string file in Directory.EnumerateFiles(directory, "XenostrapCleanup.*"))
		{
			try
			{
	...
	public static async Task HandleUpgradeAsync()
	{
		...
		{
			Xenostrap.Utility.InstallRecord.MakeExecutable(Paths.Application);
			Xenostrap.Utility.LinuxDesktopEntry.Install(Paths.Application);
		}
		if (productVersion != null)
		{
			if (Utilities.CompareVersions(productVersion, "1.0.3.6") == VersionComparison.LessThan)
			{
				...
				string path4 = Path.Combine(Paths.WindowsStartMenu, "Xenostrap");
				...
				if (Directory.Exists(path4))
				{
					...
					Xenostrap.Utility.Shortcut.Create(Paths.Application, "", StartMenuShortcut);
				}
				Registry.CurrentUser.DeleteSubKeyTree("Software\\Xenostrap", throwOnMissingSubKey: false);
				WindowsRegistry.RegisterPlayer();
			WindowsRegistry.RegisterXenostrap();
	...
	private static void ReplaceInstalledExecutable()
	{
		...
		string stagedPath = Path.Combine(directory, ".Xenostrap.update." + Guid.NewGuid().ToString("N"));
		try
		{
			File.Copy(Paths.Process, stagedPath, overwrite: false);
			Xenostrap.Utility.InstallRecord.MakeExecutable(stagedPath);
			if (Xenostrap.Utility.Platform.IsWindows)
			{
				try
				{
				...
				catch (Exception ex) when (Xenostrap.Utility.CloudFiles.IsCloudFailure(ex, Paths.Application))
				{
	...
	private static void RefreshAppIcons()
	{
		...
		try
		{
			...
			{
				Xenostrap.Utility.Shortcut.Create(Paths.Application, "", DesktopShortcut);
			}
		}
		catch
		{
	...
	public static bool RelocateInstall(string newLocation)
	{
		...
		if (!text2.EndsWith("Xenostrap", StringComparison.InvariantCultureIgnoreCase))
		{
			text2 = Path.Combine(text2, "Xenostrap");
		}
		if (string.Equals(text2, text, StringComparison.InvariantCultureIgnoreCase))
		{
			Frontend.ShowMessageBox("Xenostrap is already installed at that location.", MessageBoxImage.Asterisk);
			return false;
		}
		...
		try
		{
			...
			string path = Path.Combine(targetParent, ".Xenostrap.write." + Guid.NewGuid().ToString("N"));
			File.WriteAllText(path, "");
			File.Delete(path);
		}
		catch (Exception ex)
		...
		string stagingPath = Path.Combine(targetParent, ".Xenostrap.staging." + Guid.NewGuid().ToString("N"));
		...
		try
		{
			...
			if (Directory.Exists(text2))
			{
				replacedTarget = Path.Combine(targetParent, ".Xenostrap.previous." + Guid.NewGuid().ToString("N"));
		...
		catch (Exception ex2)
		{
			...
			Frontend.ShowMessageBox("Failed to move Xenostrap to the new location.", MessageBoxImage.Hand);
			return false;
		}
		...
		catch (Exception ex4)
		{
			...
			Frontend.ShowMessageBox("Failed to start Xenostrap from the new location.", MessageBoxImage.Hand);
	...
	private static void TryDeleteRelocationDirectory(string path, string expectedParent)
	{
		try
		{
			...
			if (Directory.Exists(fullPath) && string.Equals(parent, Path.GetFullPath(expectedParent), StringComparison.OrdinalIgnoreCase) && Path.GetFileName(fullPath).StartsWith(".Xenostrap.", StringComparison.Ordinal))
			{
