...
using Xenostrap.Integrations.RiShade;
...
namespace Xenostrap.Utility
{
    public static class ClassicIntegrations
    {
        ...
        public static void HideXenostrap()
        {
            try
            {
        ...
        private static void CreateOverlays()
        {
            try
            {
                if (Xenostrap.Integrations.Overlays.OverlayCrosshair.IsEnabled() && !Xenostrap.Integrations.Overlays.OverlayHub.CompositorCrosshairActive && Application.Current.Resources["CrosshairWindow"] is not CrosshairWindow)
                {
            ...
            try { Xenostrap.Integrations.Fullscreen.FakeExclusiveFullscreen.OnGameJoin(); } catch { }
        ...
        private static void CloseOverlays()
        {
            ...
            try { Xenostrap.Integrations.FrameGeneration.FrameGenManager.OnGameLeave(); } catch { }
            try { Xenostrap.Integrations.Fullscreen.FakeExclusiveFullscreen.OnGameLeave(); } catch { }
        }
        ...
        private static void StartRichPresence(string clientName, string map)
        {
            ...
            try
            {
                string icon = !string.IsNullOrWhiteSpace(App.Settings.Prop.UseCustomIcon)
                    ...
                    : Xenostrap.Integrations.DiscordRichPresence.GetIdleIconUrl();

                var rpc = new DiscordRpcClient("1539279631264518205");
                rpc.Initialize();
