...
internal static class WindowAudit
{
		...
		foreach (Type type in windowTypes)
		{
			...
			catch (Exception ex)
			{
				...
				for (int f = 0; f < frames.Length && shown < 6; f++)
				{
					...
					if (frame.Contains("Xenostrap", StringComparison.Ordinal))
					{
						Emit($"        {frame}");
						shown++;
					}
		...
		foreach (Type type in viewModels)
		{
			...
			catch (Exception ex)
			{
				...
				if (root is DllNotFoundException || root is EntryPointNotFoundException || root is PlatformNotSupportedException || root is TypeInitializationException)
				{
					...
					{
						if (line.Contains("Xenostrap", StringComparison.Ordinal))
						{
							Emit($"           {line.Trim()}");
							break;
						}
	...
	private static void AuditPlacement()
	{
		...
		try
		{
			...
			(double realWidth, double realHeight) = Xenostrap.Utility.ScreenMetrics.GetPrimary();
	...
	{
		Emit("icon audit:");
		foreach (Xenostrap.Enums.BootstrapperIcon icon in Enum.GetValues<Xenostrap.Enums.BootstrapperIcon>())
		{
			try
			{
				System.Windows.Media.ImageSource source = Xenostrap.Extensions.IconEx.GetIconSource(icon);
		...
		foreach (string key in new[] { "FluentSystemIcons", "FluentSystemIconsFilled" })
		{
			try
			{
				...
				{
					Emit($"  FONT  resource '{key}' -> {resolved.Source} glyphs={(Xenostrap.Utility.IconFontLoader.HasGlyphs(resolved) ? "PRESENT" : "MISSING")}");
				}
				else
				{
	...
	public static void RenderProbe(Window window, string label)
	{
		try
		{
			...
			string outPath = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "Xenostrap", "render-" + label + ".png");
