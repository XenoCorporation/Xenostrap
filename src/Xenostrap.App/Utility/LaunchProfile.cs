        ...
        public static async Task<ImageSource?> LoadAvatarAsync(string url)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    foreach (string candidate in Xenostrap.Utility.AppImage.GetCandidates(url, 96))
                    {
                        downloaded = await Xenostrap.Utility.AppImage.DownloadBytesAsync(candidate).ConfigureAwait(false);
                        if (downloaded != null)
                            break;
                    }
                    if (downloaded == null)
                ...
                if (bytes.Length == 0 || bytes.Length > MaxAvatarBytes)
                    return null;
                var bmp = Xenostrap.Utility.SafeImaging.FromBytes(bytes, 96);
                if (bmp == null)
                    return null;
                if (bmp.CanFreeze)
                    bmp.Freeze();
