...
namespace Xenostrap.Utility
{
    ...
    public static class ClassicClients
    {
        ...
        public const string DefaultBaseUrl = "https://github.com/Orc-Archive/Orc-Xenostrap/releases/tag/v1/";

        public const string LegacyDefaultBaseUrl = "https://github.com/xenostrap/RobloxClients/raw/main/";

        public const string WebServerExe = "XenostrapClient.WebServer.exe";

        public const string ConfigFileName = "XenostrapClient_Config.json";
        ...
        public static async Task<ClassicManifest?> FetchManifestAsync(CancellationToken ct, bool force = false)
        {
            ...
            try
            {
                ...
                foreach (string url in BuildCandidateUrls("manifest.json"))
                {
                    try
                    {
                        string json = await Xenostrap.Utility.Http.GetStringBoundedAsync(Http, url, ct, MaxReleaseMetadataBytes).ConfigureAwait(false);
        ...
        public static string UserAppData => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "XenostrapClientData");

        public static string WebServerUserAppData => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "XenostrapClient");
        ...
        private const string RenamedHelperName = "XenostrapClientHelper.dll";
        ...
        private static void FixHelperFolderString(string helperPath)
        {
            try
            {
                ...
                byte[] broken = Encoding.ASCII.GetBytes("XenostrapClient\0\0\0\0");
                byte[] fixedBytes = Encoding.ASCII.GetBytes("XenostrapClientData");
        ...
        private static async Task<ReleaseAssetIntegrity> DownloadAndExtractAsync(string relativePath, Action<double, string> progress, CancellationToken ct)
        {
            ...
            string tempZip = Path.Combine(Path.GetTempPath(), "xenostrap_client_" + Guid.NewGuid().ToString("N") + ".zip");
        ...
        public static async Task ResolveRobloxUserAsync(CancellationToken ct)
        {
            try
            {
                var account = await Xenostrap.Integrations.RobloxCookie.GetAccountAsync(ct).ConfigureAwait(false);
        ...
        public static Process? Launch(string client, ClientLaunchType launchType, ushort port = DefaultPort, string ip = "localhost", Action<string>? status = null)
        {
            ...
            Xenostrap.Integrations.AssetProxy.AssetProxyServer.DisableForUnsupportedClient();
            Xenostrap.Integrations.Studio.StudioPluginInstaller.StashForClassicClient();

            string? redirectFailure = Xenostrap.Integrations.ClassicHostRedirect.Set(enable: true);
        ...
        public static void ShutdownAll()
        {
            ...
            try { Xenostrap.Integrations.ClassicHostRedirect.RemoveIfStale(); }
            catch (Exception ex) { App.Logger?.WriteLine("ClassicClients::ShutdownAll", "Host redirect cleanup failed: " + ex.Message); }

            try { Xenostrap.Integrations.Studio.StudioPluginInstaller.RestoreAfterClassicClient(); }
        ...
        {
            Process[] existing = Process.GetProcessesByName("XenostrapClient.WebServer");
            int aliveCount = 0;
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                return result + " Xenostrap also tried to install the runtime automatically but could not: " + ex.Message;
            }
        ...
        private static string? LaunchWebServerOnce(string client, string? runtimeRoot, out bool hostFailure)
        {
            ...
            try
            {
                ...
                start.ArgumentList.Add("--prefixes=" + Xenostrap.Integrations.ClassicHostRedirect.ServerPrefix);
                ...
                App.Logger?.WriteLine("ClassicClients::EnsureWebServerRunning", "Started the ORC local service on " + Xenostrap.Integrations.ClassicHostRedirect.ServerPrefix);
                for (int i = 0; i < 40; i++)
                {
                    if (server.HasExited)
                    {
        ...
        {
            string installerPath = Path.Combine(Path.GetTempPath(), "xenostrap_aspnetcore_" + Guid.NewGuid().ToString("N") + ".exe");
            try
            {
        ...
        private static void InstallPrivateRuntime(string privateRoot, string arch, int major, Action<string>? status)
        {
            ...
            string zipPath = Path.Combine(Path.GetTempPath(), "xenostrap_aspnetcore_" + Guid.NewGuid().ToString("N") + ".zip");
        ...
        private static string DescribeServiceFailure(string detail)
        {
            ...
            bool running = false;
            foreach (Process existing in Process.GetProcessesByName("XenostrapClient.WebServer"))
            {
            ...
            if (!Xenostrap.Integrations.ClassicHostRedirect.IsUrlAclReserved())
                return "Windows has not reserved " + Xenostrap.Integrations.ClassicHostRedirect.ServerPrefix + " for the local service. Relaunch and accept the elevation prompt.";
            if (!running)
                return "The ORC local service is not running. Restart Xenostrap and try again.";
        ...
        {
            foreach (Process server in Process.GetProcessesByName("XenostrapClient.WebServer"))
            {
