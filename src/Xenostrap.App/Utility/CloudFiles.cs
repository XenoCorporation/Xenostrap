...
internal static partial class CloudFiles
{
	...
	public static bool IsCloudFailure(Exception exception, string? path)
	{
		...
		return IsPlaceholder(path) || Xenostrap.Installer.IsCloudSyncedPath(path);
	}
	...
	{
		if (!Platform.IsWindows || !Paths.Initialized || !Xenostrap.Installer.IsCloudSyncedPath(Paths.Base))
		{
			return;
		}
