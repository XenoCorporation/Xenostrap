    ...
    public static class WebsiteProfileEditor
    {
        ...
        public static async Task<(bool Ok, bool Queued, string? Message)> SaveAsync(WebsiteProfileData d)
        {
            try
            {
                ...
                {
                    return (false, true, "Cloudflare (the service that hosts Xenostrap) is temporarily unavailable, so your profile could not be saved right now. This is not something you did. Your changes are queued and will be applied automatically once Cloudflare recovers, even if you close this window.");
                }
