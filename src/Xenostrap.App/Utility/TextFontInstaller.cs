...
""";
	...
	public static void Install()
	{
		try
		{
			if (OperatingSystem.IsLinux())
			{
				...
				string fontDirectory = Path.Combine(home, ".local", "share", "fonts", "xenostrap");
	...
	private static bool ExtractFonts(string directory)
	{
		...
		foreach (string fileName in FontFiles)
		{
			try
			{
				...
				using Stream? source = assembly.GetManifestResourceStream("Xenostrap.Resources.Fonts." + fileName);
				if (source == null)
				{
					continue;
				}
	...
	private static bool WriteAliasConfig(string home)
	{
		try
		{
			...
			string configPath = Path.Combine(configDirectory, "60-xenostrap-segoe.conf");
