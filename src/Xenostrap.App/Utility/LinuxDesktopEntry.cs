...
{
	private const string EntryFileName = "xenostrap.desktop";

	private const string IconName = "xenostrap";
	...
	public static void Install(string executablePath)
	{
		...
		try
		{
			...
			string contents = string.Join('\n',
				...
				"Terminal=false",
				"Categories=Game;",
				"Keywords=roblox;xenostrap;bootstrapper;",
				"StartupNotify=true",
				"StartupWMClass=Xenostrap",
	...
	private static void WriteIcon(string iconPath)
	{
		try
		{
			StreamResourceInfo? info = Application.GetResourceStream(new Uri("pack://application:,,,/Xenostrap.png", UriKind.Absolute));
			if (info?.Stream == null)
			{
				return;
			}
