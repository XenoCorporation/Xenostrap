...
namespace Xenostrap.Desktop;

public sealed class PlatformOverviewWindow : Window
{
	private static readonly string[] ThemeValues = ["Default", "Dark", "Light", "Xenostrap", "UltraGray", "Berry", "Blue", "Cyan", "Green", "Orange", "Pink", "Purple", "Red", "Yellow", "Custom"];
		...
		foreach (string scheme in schemes)
		{
			...
			try
			{
				result = await _host.ProtocolRegistration.RegisterAsync(
					new ProtocolRegistrationRequest(scheme, applicationPath, "Xenostrap"),
	...
	private async Task SendTestNotificationAsync()
	{
		OperationResult result = await _host.Notifications.ShowAsync(
			new NotificationRequest("Xenostrap", "Native notification support is active."),
			_lifetimeCancellation.Token);
	...
	private static ThemePalette GetThemePalette(string theme)
	{
		...
		return theme switch
		{
			...
			"Xenostrap" => new ThemePalette(Color.FromRgb(19, 7, 36), Color.FromRgb(30, 11, 47)),
