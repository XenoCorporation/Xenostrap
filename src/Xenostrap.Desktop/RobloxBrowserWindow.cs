...
(() => {
	if (window.__xenostrapBridgeReady) {
		return;
	}
	const send = payload => {
	...
	window.__xenostrapPostMessage = send;
	window.__xenostrapBridgeReady = true;
})();
""";
	...
	private async Task<OperationResult> SynchronizeLibraryPinsAsync()
	{
		...
		OperationResult<string> execution = await _webViewHost.ExecuteScriptAsync(
			"window.__vsLibraryPins = " + payload + "; window.dispatchEvent(new CustomEvent('xenostrapLibraryPinsChanged'));",
			_lifetimeCancellation.Token);
